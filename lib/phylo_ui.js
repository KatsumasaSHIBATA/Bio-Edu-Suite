...
// --- ツールチップ機能 (ガイドライン第6項④ 準拠) ---
    ...
    // ==========================================
    // 同一セッション内ステート保持＆完全ハイドレーション規格 (第7.4項準拠)
    // ==========================================
    ...
    function savePhyloWorkspace() {
        ...
        // 系統樹が完成しているかどうかの厳密な判定
        const isCompleted = Boolean(
            clusters && clusters.length > 0 && 
            clusters.some(c => c && c.active) && 
            originalNames && originalNames.length > 0 &&
            (clusters.filter(c => c && c.active).length === 1 || clusters.length === 1)
        );

        // 【安全弁】すでに完成データが保存されている場合、手動初期化時（isResetting）以外は未完了データでの上書き破壊を禁止
    ...
    function restorePhyloWorkspace() {
        ...
        try {
            ...
            if (data.hasCalculated && data.clusters && data.clusters.length > 0) {
                ...
                // 系統樹Canvasの完全描画（タイミング競合を防止するため多段実行）
                drawTree();
