...
// 起動直後から復元完了まで、不意のイベント発火によるデータ破壊を完全遮断
...
// --- ツールチップ機能 (ガイドライン第6項④ 準拠) ---
    ...
    function switchAlgo() {
        if (window.isPhyloHydrating || (typeof window.isHydrating !== 'undefined' && window.isHydrating)) return; // ハイドレーション中の自動リセットを物理遮断
        
        showToast("アルゴリズムの設定を変更しました。「▶ 実行」ボタンを押すと再計算されます。", "info");
    ...
    // ==========================================
    function updatePhyloDynamicInsight() {
        ...
        if (typeof clusters === 'undefined' || clusters.length === 0 || !clusters.some(c => c.active)) {
            badge.style.background = '#f1f5f9';
            badge.style.color = '#64748b';
            badge.style.border = '1px solid #e2e8f0';
            badge.innerText = '解析待機中';
            content.innerHTML = '<p style="margin: 0; color: var(--text-muted);">データ待機中：系統樹を構築すると、選択されたアルゴリズム、配列品質（ギャップ率）、枝の長さの分布、ブートストラップ値の統計に基づく具体的な着眼点がここに表示されます。</p>';
            return;
        }

        const algo = document.getElementById('algoSelect').value;
        let algoName = "UPGMA法";
        let algoCharacteristic = "";

        if (algo === 'UPGMA') {
            algoName = "UPGMA法 (平均距離法)";
            algoCharacteristic = "すべての分類群が「同じ速度で進化した（分子時計）」と仮定しています。共通祖先から現在の各生物までの枝の長さ（横幅）が均等に揃っている点を確認してください。";
        } else if (algo === 'NJ') {
            algoName = "NJ法 (近隣結合法)";
            algoCharacteristic = "系統ごとに「塩基置換速度が異なる」ことを考慮して計算しています。枝の長さがバラバラであり、横に長く伸びている枝ほど、その系統で多くの突然変異が蓄積したことを示しています。";
        } else if (algo === 'MP') {
            algoName = "MP法 (最大節約法)";
            algoCharacteristic = "「1つ1つの塩基の違い」を直接評価し、進化の過程で生じた突然変異の総回数が最も少なくなる樹形を探索しています。枝の長さは推定された突然変異の回数（ステップ数）を表します。";
        } else if (algo === 'ML') {
            algoName = "ML法 (最尤法)";
            algoCharacteristic = "塩基置換モデルに基づき、現在の塩基配列が得られる確率（尤度）が最大となる樹形を探索しています。枝の長さは1サイトあたりの平均塩基置換数を表します。";
        }

        // 配列ギャップ率の計算
        let gapRatio = 0;
        if (typeof originalSeqs !== 'undefined' && originalSeqs.length > 0) {
            let totalChars = 0, gapChars = 0;
            originalSeqs.forEach(s => {
                totalChars += s.seq.length;
                let gaps = s.seq.match(/[-N?]/g);
                if (gaps) gapChars += gaps.length;
            });
            gapRatio = totalChars > 0 ? (gapChars / totalChars * 100).toFixed(1) : 0;
        }

        // ブートストラップ設定の確認
        let bsVal = 0;
        const checkedBS = document.querySelector('input[name="bootstrap"]:checked');
        if (checkedBS) bsVal = parseInt(checkedBS.value, 10) || 0;
        ...
        if (gapRatio > 10) {
            ...
            badge.innerText = `${algo} / ギャップ高め (${gapRatio}%)`;
        } else if (bsVal > 0) {
            badge.style.background = '#ecfdf5';
            badge.style.color = '#047857';
            badge.style.border = '1px solid #a7f3d0';
            badge.innerText = `${algo} / BS検証済 (${bsVal}回)`;
        } else {
            badge.style.background = '#f0fdf4';
            badge.style.color = '#15803d';
            badge.style.border = '1px solid #bbf7d0';
            badge.innerText = `${algo} / 構築完了`;
        }

        // データの品質テキスト
        let qualityText = "";
        if (gapRatio > 10) {
            qualityText = `<p style="margin-bottom: 6px; color: #b91c1c;"><strong>配列データの品質注意:</strong> 入力データに含まれる欠失（ギャップ）や未確定塩基（N）の割合が <strong>${gapRatio}%</strong> と高めです。ノイズによる誤ったクラスタリングを防ぐため、配列両端のトリミングを検討してください。</p>`;
        } else {
            qualityText = `<p style="margin-bottom: 6px;"><strong>配列データの品質:</strong> 欠失（ギャップ）や未確定塩基（N）の割合は <strong>${gapRatio}%</strong> であり、解析に十分な品質が保たれています。</p>`;
        }

        // ブートストラップ解析の着眼点
        let bsInsightText = "";
        if (bsVal > 0) {
            bsInsightText = `<p style="margin-bottom: 0;"><strong>信頼性の検証:</strong> 各分岐ノードの支持率（ブートストラップ値 %）を点検してください。70%以上の分岐は統計的に堅牢なグループ化ですが、50〜60%台の低い分岐は塩基配列の僅かな偶然で変わり得るため、慎重な解釈が必要です。</p>`;
        } else {
            bsInsightText = `<p style="margin-bottom: 0;"><strong>次のステップ:</strong> より確かな系統関係を議論するために、左側の「ブートストラップ解析」を有効にして再計算し、各分岐の信頼性（%）を検証してみましょう。</p>`;
        }

        content.innerHTML = `
            <p style="margin-bottom: 6px;"><strong>アルゴリズムの特性 (${algoName}):</strong> ${algoCharacteristic}</p>
            ${qualityText}
            ${bsInsightText}
        `;
    }
    ...
    function updateExplanation() {
        ...
        const diagPanel = document.getElementById('autoDiagPanel');
        if (diagPanel) diagPanel.style.display = 'none';

        updatePhyloDynamicInsight();
    }
    ...
    // --- 4案統合ハイブリッド・アーキテクチャ ヘルパー関数群 ---
    function formatCleanTaxonName(name) {
        if (!name) return '';
        let original = name.trim();
        let clean = original.replace(/^>\s*/, '');
        // タイムスタンプ型プレフィックスの除去 (例: 20260924_145558_ または 20260924-145558_)
        clean = clean.replace(/^\d{4,8}[_\-]\d{4,6}[_\-]?/, '');
        // 一般的なサンプルID接頭辞の除去 (直後に和名・学名が続く場合: 例 S01_ヒト, Sample_ニワトリ)
        clean = clean.replace(/^[A-Za-z0-9_\-]+_(?=[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fffA-Z])/, '');
        return formatScientificName(clean);
    }

    function formatShortColLabel(name, index) {
        let cleaned = formatCleanTaxonName(name);
        let shortText = cleaned.length > 5 ? cleaned.substring(0, 5) + '..' : cleaned;
        return `[${index + 1}] ${shortText}`;
    }

    function setMatrixCrosshair(row, col, active) {
        const table = document.getElementById('matrixTable');
        if (!table) return;
        
        // 既存のクロスヘアクラスを一括解除
        table.querySelectorAll('.crosshair-highlight').forEach(el => el.classList.remove('crosshair-highlight'));
        table.querySelectorAll('.crosshair-header').forEach(el => el.classList.remove('crosshair-header'));

        if (active) {
            // 対象行・列のセルにハイライトを付与
            table.querySelectorAll(`[data-m-row="${row}"]`).forEach(el => el.classList.add('crosshair-highlight'));
            table.querySelectorAll(`[data-m-col="${col}"]`).forEach(el => el.classList.add('crosshair-highlight'));
            
            // ヘッダーにハイライトを付与
            const rHead = table.querySelector(`[data-m-rhead="${row}"]`);
            if (rHead) rHead.classList.add('crosshair-header');
            const cHead = table.querySelector(`[data-m-chead="${col}"]`);
            if (cHead) cHead.classList.add('crosshair-header');
            // 交差するセル自体も強調
            const targetCell = table.querySelector(`[data-m-row="${row}"][data-m-col="${col}"]`) || table.querySelector(`[data-m-row="${col}"][data-m-col="${row}"]`);
            if (targetCell) targetCell.classList.add('crosshair-highlight');
        }
    }
    ...
    function formatScientificName(name) {
        ...
        let clean = originalName.replace(/^>\s*/, '');
        
        let geneSymbol = '';
        const geneMatch = clean.match(/\(([-A-Za-z0-9_]+)\)\s*$/);
        if (geneMatch) {
            geneSymbol = geneMatch[1];
        }
        
        const ncbiPattern = /[A-Z]{1,2}_?\d{4,}/i;
        const scientificNamePattern = /^[A-Z][a-z]+[\s_]+[a-z]+/;
        
        if (ncbiPattern.test(clean) || scientificNamePattern.test(clean)) {
            let cleanText = clean.replace(/[A-Z]{1,2}_?\d{4,}(\.\d+)?[:\s]*/ig, '').replace(/_/g, ' ').trim();
            let parts = cleanText.split(/\s+/).filter(p => p.length > 0);
            
            if (parts.length >= 2) {
                let p0 = parts[0].replace(/[^a-zA-Z]/g, '');
                let p1 = parts[1].replace(/[^a-zA-Z]/g, '');
                if (p0.length > 0 && p1.length > 0) {
                    let genus = p0.charAt(0).toUpperCase();
                    let species = p1.toLowerCase();
                    let baseName = `${genus}. ${species}`;
                    return geneSymbol ? `${baseName}_${geneSymbol}` : baseName;
                }
            }
            return cleanText.length > 0 ? cleanText : originalName;
        }
    ...
    function toggleUIState(isProcessing) {
        window.isPhyloCalculating = isProcessing;
        ...
        if (isProcessing) {
            ...
            if (bsRadios) bsRadios.forEach(r => r.disabled = false);
            if (algo === 'MP') {
                if (bs1000) bs1000.disabled = true;
            } else if (algo === 'ML') {
                if (bs100) bs100.disabled = true;
                if (bs1000) bs1000.disabled = true;
            }
        }
    }
    ...
    function handleExecute() {
        ...
        if (!algoSelect || !fastaInput) return;
        const algo = algoSelect.value;
        const fastaText = fastaInput.value.trim();
        
        if (!fastaText) {
            showToast("アライメント済みのFASTAデータを入力してください", "warning");
            return;
        }

        // 実行ボタン自体はロックせず、二重連打防止のみとし、処理終了後に必ず即座に復帰
        toggleUIState(true);
        ...
        try {
            ...
            originalNames.forEach(n => {
                ...
                opt.text = `外群: ${formatCleanTaxonName(n)}`;
            ...
            document.getElementById('btnToggleHeatmap').disabled = false;
            ...
            // toggleUIState(false) は計算・結合完了時まで遅延
    ...
    function executeMP(fastaText, result) {
        ...
        document.getElementById('btnNextStep').style.display = 'none';
        const btnTH1 = document.getElementById('btnToggleHeatmap');
        if (btnTH1) btnTH1.disabled = true;
    ...
    function executeML(fastaText, result) {
        ...
        document.getElementById('btnNextStep').style.display = 'none';
        document.getElementById('btnExportCSV').disabled = true;
        const btnTH2 = document.getElementById('btnToggleHeatmap');
        if (btnTH2) btnTH2.disabled = true;
    ...
    // --- ステート・タイムマシン復元機能 ---
    function cloneTreeSnapshot(node) {
        if (!node) return null;
        const cloned = { ...node, parent: null }; // 循環参照を防止
        if (node.left) cloned.left = cloneTreeSnapshot(node.left);
        if (node.right) cloned.right = cloneTreeSnapshot(node.right);
        return cloned;
    }

    function highlightLogEntry(runId) {
        const tbody = document.getElementById('logTableBody');
        if (!tbody) return;
        const rows = tbody.querySelectorAll('tr');
        rows.forEach(r => r.classList.remove('active-log'));
        const targetRow = document.getElementById(`log_row_${runId}`);
        if (targetRow) targetRow.classList.add('active-log');
    }

    window.restoreTreeFromLog = function(runId) {
        const log = analysisLogs.find(l => l.id === runId);
        if (!log || !log.snapshot) {
            showToast("該当する解析ログが見つかりません", "warning");
            return;
        }
        
        const snap = log.snapshot;
        currentTreeRoot = cloneTreeSnapshot(snap.treeRoot);
        if (currentTreeRoot) currentTreeRoot.active = true;
        if (typeof restoreParentLinks === 'function') {
            restoreParentLinks(currentTreeRoot);
        }
        
        // 描画エンジンおよびエクスポートに必要な全変数を過去ステートへ完全再充填
        clusters = [currentTreeRoot];
        if (clusters[0]) clusters[0].active = true;
        originalNames = [...snap.sampleNames];
        originalSeqs = snap.sampleNames.map(n => ({ name: n, seq: '' }));
        window.currentSampleNames = [...snap.sampleNames];
        window.currentDistanceMatrix = snap.distMatrix.map(row => [...row]);
        distanceMatrix = snap.distMatrix.map(row => [...row]);
        initialDistanceMatrix = snap.distMatrix.map(row => [...row]);
        
        // UIコントロール（アルゴリズム表示・外群選択）の同期
        const algoSelect = document.getElementById('algoSelect');
        if (algoSelect && snap.algo) {
            algoSelect.value = snap.algo;
        }
        ...
        if (outgroupSelect && originalNames.length > 0) {
            ...
            outgroupSelect.disabled = false;
            if (snap.outgroup) {
                outgroupSelect.value = snap.outgroup;
            }
        }
        
        // 全エクスポートボタン（Newickコピー、TSVコピー、PNG保存）の完全活性化
        updatePhyloExportButtonsState(true);
        ...
        highlightLogEntry(runId);
        if (typeof updateExplanation === 'function') updateExplanation();
        drawTree();
        
        let algoName = snap.algo === 'UPGMA' ? '平均距離法 (UPGMA法)' : (snap.algo === 'NJ' ? '近隣結合法 (NJ法)' : (snap.algo === 'MP' ? '最大節約法 (MP法)' : '最尤法 (ML法)'));
        showToast(`実行回 ${runId} (${algoName}) の系統樹を復元しました`, "info");
        
        if (!window.isPhyloHydrating && typeof savePhyloWorkspace === 'function') {
            savePhyloWorkspace();
        }
    };
    ...
    function addLogEntry(algo, bsVal, rootNode) {
        ...
        let sNames = window.currentSampleNames ? [...window.currentSampleNames] : (typeof originalNames !== 'undefined' && originalNames.length > 0 ? [...originalNames] : []);
        let dMatrix = window.currentDistanceMatrix ? window.currentDistanceMatrix.map(row => [...row]) : (typeof initialDistanceMatrix !== 'undefined' && initialDistanceMatrix.length > 0 ? initialDistanceMatrix.map(row => [...row]) : []);

        // ステートスナップショットの作成
        let snapshot = {
            treeRoot: cloneTreeSnapshot(rootNode),
            algo: algo,
            bsVal: bsVal,
            outgroup: document.getElementById('outgroupSelect')?.value || "",
            sampleNames: sNames,
            distMatrix: dMatrix
        };
        
        analysisLogs.push({ id: runId, algo: algo, bs: bsStr, clades: cladeCsv, snapshot: snapshot });
        
        let tbody = document.getElementById('logTableBody');
        if (runId === 1) tbody.innerHTML = ''; 

        let tr = document.createElement('tr');
        tr.className = 'clickable-log';
        tr.id = `log_row_${runId}`;
        tr.setAttribute('onclick', `restoreTreeFromLog(${runId})`);
    ...
    // ログテーブルの確実なクリックイベント補足（イベントデリゲーション）
    document.getElementById('logTableBody')?.addEventListener('click', (e) => {
        const row = e.target.closest('tr.clickable-log');
        if (!row || !row.id) return;
        const m = row.id.match(/^log_row_(\d+)$/);
        if (m) {
            const runId = parseInt(m[1], 10);
            if (typeof window.restoreTreeFromLog === 'function') {
                window.restoreTreeFromLog(runId);
            }
        }
    });
    ...
    function swapNodeBranches(node) {
        if (!node) return;
        let temp = node.left;
    ...
    function findInternalNodeAt(node, x, y, hitRadius = 12) {
        ...
        let foundLeft = findInternalNodeAt(node.left, x, y, hitRadius);
        if (foundLeft) return foundLeft;
        return findInternalNodeAt(node.right, x, y, hitRadius);
    }

    function initTreeCanvasInteractions() {
        const canvas = document.getElementById('treeCanvas');
        if (!canvas) return;
        ...
        canvas.addEventListener('mousemove', (e) => {
            ...
            let target = findInternalNodeAt(currentTreeRoot, x, y, 12);
            if (target) {
                canvas.style.cursor = 'pointer';
                if (hoveredInternalNode !== target) {
                    hoveredInternalNode = target;
                    drawTree();
                }
            } else {
                canvas.style.cursor = 'default';
                if (hoveredInternalNode !== null) {
                    hoveredInternalNode = null;
                    drawTree();
                }
            }
        });
        ...
        canvas.addEventListener('click', (e) => {
            ...
            let target = findInternalNodeAt(currentTreeRoot, x, y, 14);
            if (target) {
                swapNodeBranches(target);
                if (typeof triggerHapticFeedback === 'function') {
                    triggerHapticFeedback();
                }
                showToast("枝を回転しました（分岐トポロジーは不変です）", "info");
                drawTree();
                if (!window.isPhyloHydrating && typeof savePhyloWorkspace === 'function') {
                    savePhyloWorkspace();
                }
    ...
    function generateNewick(node) {
        ...
        if (!node.left && !node.right) {
            return formatCleanTaxonName(node.name).replace(/\s+/g, '_') + (node.edge !== undefined ? ":" + node.edge.toFixed(5) : "");
        }
    ...
    // 系統樹から指定した葉ノード（外群）を切り離し、内群のみの部分木を抽出するヘルパー関数
    function pruneLeafNode(node, targetLeafName) {
        if (!node) return null;
        if (!node.left && !node.right) {
            return node.name === targetLeafName ? null : node;
        }
        if (node.left && !node.left.left && !node.left.right && node.left.name === targetLeafName) {
            return node.right;
        }
        if (node.right && !node.right.left && !node.right.right && node.right.name === targetLeafName) {
            return node.left;
        }
        return {
            ...node,
            left: pruneLeafNode(node.left, targetLeafName),
            right: pruneLeafNode(node.right, targetLeafName)
        };
    }

    function copyNewick() {
        let outgroupName = document.getElementById('outgroupSelect').value;
        let root = currentTreeRoot || clusters.find(c => c.active);
        ...
        let targetRoot = root;
        if (outgroupName) {
            let routed = getRoutedTree(root, outgroupName);
            // 有根樹の根直下にある外群ノードを除去し、内群サブツリーを取り出す
            if (routed.left && routed.left.name === outgroupName) {
                targetRoot = routed.right;
            } else if (routed.right && routed.right.name === outgroupName) {
                targetRoot = routed.left;
            } else {
                targetRoot = pruneLeafNode(routed, outgroupName) || routed;
            }
        }
        
        let newick = generateNewick(targetRoot) + ";";
        
        navigator.clipboard.writeText(newick).then(() => {
            if (outgroupName) {
                showToast(`外群（${formatScientificName(outgroupName)}）を除外した系統樹(Newick形式)をコピーしました`, "success");
            } else {
    ...
    window.copyMutationDataForApp11 = function() {
        let sampleNames = window.currentSampleNames || (typeof originalNames !== 'undefined' && originalNames.length > 0 ? [...originalNames] : null);
        let distMat = window.currentDistanceMatrix || (typeof initialDistanceMatrix !== 'undefined' && initialDistanceMatrix.length > 0 ? initialDistanceMatrix.map(row => [...row]) : null);
        ...
        const outgroupName = document.getElementById('outgroupSelect')?.value || "";
        const timestamp = new Date().toISOString();
        
        let targetNames = [...sampleNames];
        let targetMat = distMat.map(row => [...row]);

        // 外群が指定されている場合は除外
        if (outgroupName) {
            const outgroupIndex = targetNames.indexOf(outgroupName);
            if (outgroupIndex !== -1) {
                targetNames = targetNames.filter((_, idx) => idx !== outgroupIndex);
                targetMat = targetMat
                    .filter((_, rIdx) => rIdx !== outgroupIndex)
                    .map(row => row.filter((_, cIdx) => cIdx !== outgroupIndex));
            }
        }

        if (targetNames.length < 2) {
            showToast("外群除外後のサンプル数が不足しています", "warning");
            return;
        }

        let metaHeader = `# source: App_8_Phylogenetic_Tree_Builder | type: mutation | timestamp: ${timestamp}`;
        let lines = [metaHeader];

        // 内群サンプルによる2次元突然変異特徴量（PCAまたは距離軸）の算出
        let scores = [];
        if (typeof nipalsPCA === 'function' && targetMat.length >= 2 && targetMat[0].length >= 2) {
            try {
                const pcaRes = nipalsPCA(targetMat, 2);
        ...
        targetNames.forEach((name, i) => {
            ...
            if (scores.length > i && scores[i].length >= 2) {
                ...
                x = (targetMat[i][0] || 0).toFixed(4);
                y = (targetMat[i][1] !== undefined ? targetMat[i][1] : (targetMat[i][0] || 0)).toFixed(4);
            }
            lines.push(`${typeof formatCleanTaxonName === 'function' ? formatCleanTaxonName(name) : name}\t${x}\t${y}`);
        });

        const tsvText = lines.join('\n');
        navigator.clipboard.writeText(tsvText).then(() => {
            if (outgroupName) {
                showToast(`外群（${formatScientificName(outgroupName)}）を除外した⑪用 突然変異データをコピーしました`, "success");
            } else {
    ...
    function renderMatrix() {
        ...
        if (isHeatmapActive) {
            for (let i = 0; i < clusters.length; i++) {
                ...
                for (let j = i + 1; j < clusters.length; j++) {
                    if (!clusters[j].active) continue;
                    let v = displayMatrix[i][j];
                    if (v < minVal) minVal = v;
                    if (v > maxVal) maxVal = v;
                }
            }
            if (minVal === Infinity) { minVal = 0; maxVal = 1; }
            if (minVal === maxVal) { maxVal = minVal + 1e-5; }
        }

        let html = "<table class=\"matrix-table\" id=\"matrixTable\"><tr><th class=\"matrix-th-corner\"></th>";
        for (let j = 0; j < clusters.length; j++) {
            let fullClean = formatCleanTaxonName(originalNames[j]);
            let shortCol = formatShortColLabel(originalNames[j], j);
            let headerCls = clusters[j].active ? 'scientific-name matrix-th-col' : 'scientific-name matrix-th-col matrix-inactive-header';
            html += `<th class="${headerCls}" data-tooltip="${fullClean}" data-m-chead="${j}">${shortCol}</th>`;
        }
        html += "</tr>";
        
        for(let i=0; i<clusters.length; i++) {
            let fullClean = formatCleanTaxonName(originalNames[i]);
            let rowLabel = `[${i + 1}] ${fullClean}`;
            let headerCls = clusters[i].active ? 'scientific-name matrix-th-row' : 'scientific-name matrix-th-row matrix-inactive-header';
            html += `<tr><th class="${headerCls}" data-tooltip="${fullClean}" data-m-rhead="${i}">${rowLabel}</th>`;
            for(let j=0; j<clusters.length; j++) {
                if(i === j) {
                    html += `<td class="matrix-diagonal matrix-cell" data-m-row="${i}" data-m-col="${j}">-</td>`;
                } else if(j < i) {
                    html += `<td class="matrix-diagonal matrix-cell" data-m-row="${i}" data-m-col="${j}"></td>`; 
                } else {
                    ...
                    if(!clusters[i].active || !clusters[j].active) {
                        html += `<td class="matrix-inactive matrix-cell" data-m-row="${i}" data-m-col="${j}">${val}</td>`;
                    } else {
                        let isSelected = selectedPair && ((i===selectedPair.i && j===selectedPair.j) || (i===selectedPair.j && j===selectedPair.i));
                        let cls = isSelected ? 'matrix-min matrix-cell' : 'matrix-clickable matrix-cell';
                        
                        let styleAttr = '';
                        if (isHeatmapActive) {
                            let v = displayMatrix[i][j];
                            let ratio = (v - minVal) / (maxVal - minVal);
                            ratio = Math.max(0, Math.min(1, ratio));
                            let alpha = 0.12 + (1.0 - ratio) * 0.43; // 0.12 から 0.55
                            styleAttr = ` style="background-color: rgba(26, 188, 156, ${alpha.toFixed(3)});"`;
                        }
                        
                        html += `<td id="matrix_cell_${i}_${j}" class="${cls}" data-m-row="${i}" data-m-col="${j}" onmouseenter="setMatrixCrosshair(${i}, ${j}, true)" onmouseleave="setMatrixCrosshair(${i}, ${j}, false)" onclick="checkUserSelection(${i}, ${j})"${styleAttr}>${val}</td>`;
                    }
                }
            }
            html += "</tr>";
    ...
    function checkUserSelection(i, j) {
        ...
        if(isCorrect) {
            ...
            document.getElementById('statusLog').innerHTML = `[正解！] <span class="highlight-step scientific-name" style="color:#f1c40f;">${formatScientificName(clusters[i].name)}</span> と <span class="highlight-step scientific-name" style="color:#f1c40f;">${formatScientificName(clusters[j].name)}</span> が一番近いですね。<br>自動的に結合して行列を更新します。`;
            document.getElementById('btnNextStep').style.display = 'none';
            renderMatrix();
    ...
    function executeCombine() {
        ...
        if(remainingNodes <= 1) {
            renderMatrix(); 
            currentTreeRoot = clusters.find(c => c.active);
    ...
    function changeOutgroup() {
        let outgroupName = document.getElementById('outgroupSelect').value;
        let baseRoot = clusters.find(c => c.active) || clusters[0];
        if (baseRoot) {
            currentTreeRoot = outgroupName ? getRoutedTree(baseRoot, outgroupName) : baseRoot;
        }
        drawTree();
        if (!window.isPhyloHydrating) {
            savePhyloWorkspace();
        }
    }
    ...
    function drawTree() {
        ...
        if (activeClusters.length === 1) {
            if (!currentTreeRoot) {
                currentTreeRoot = outgroupName ? getRoutedTree(activeClusters[0], outgroupName) : activeClusters[0];
            }
            forest = [currentTreeRoot];
        }
        ...
        originalNames.forEach(name => {
            let tw = ctx.measureText(formatCleanTaxonName(name)).width;
        ...
        function draw(node) {
            ...
            if(!node.left && !node.right) {
                ...
                ctx.fillText(formatCleanTaxonName(node.name), x + 12, y);
            ...
            if (node.left && node.right) {
                ...
                // 1. 枝の線を描画
                ctx.strokeStyle = '#27ae60'; ctx.lineWidth = 3; ctx.lineJoin = 'round'; ctx.beginPath();
                ...
                // 2. 内部ノードの丸を描画
                let isHovered = hoveredInternalNode && hoveredInternalNode.id === node.id;
                ctx.beginPath(); ctx.arc(x, y, isHovered ? 7 : 5, 0, Math.PI*2); ctx.fillStyle = isHovered ? '#f1c40f' : '#e67e22'; ctx.fill(); ctx.stroke();
                ...
                let leftMidX = (x + lx) / 2;
                let leftMidY = ly - 4;
                ctx.strokeText(leftEdgeVal, leftMidX, leftMidY);
                ctx.fillStyle = '#2c3e50';
                ctx.fillText(leftEdgeVal, leftMidX, leftMidY);

                // 右枝長
                let rightMidX = (x + rx) / 2;
                let rightMidY = ry - 4;
                ctx.strokeText(rightEdgeVal, rightMidX, rightMidY);
                ctx.fillStyle = '#2c3e50';
                ctx.fillText(rightEdgeVal, rightMidX, rightMidY);
                ...
                if (node.bootstrap !== undefined && !isRootOfForest) {
                    ...
                    let bsText = node.bootstrap + "%";
                    let bsX = x - 7;
                    let bsY = y + 4;
                    ctx.strokeText(bsText, bsX, bsY);
                    ctx.fillStyle = '#16a085';
                    ctx.fillText(bsText, bsX, bsY);
                }
            }
        }
        ...
        function checkBS(node) {
            if (!node || hasBootstrap) return;
            let isRootOfForest = forest.some(r => r.id === node.id);
            if (node.bootstrap !== undefined && !isRootOfForest) {
                hasBootstrap = true;
                return;
            }
            if (node.left) checkBS(node.left);
            if (node.right) checkBS(node.right);
        }
        forest.forEach(root => checkBS(root));

        // 凡例のテキストと寸法計算（右下に安全に配置）
        ctx.font = '11px sans-serif';
        let branchLabel = algoVal === 'MP' ? 'Mutations (Steps)' : (algoVal === 'ML' ? 'Substitutions/Site' : 'Branch Length');
        let w1 = ctx.measureText(branchLabel).width;
        let w2 = hasBootstrap ? ctx.measureText('% Bootstrap').width : 0;
        let legendW = hasBootstrap ? Math.round(w1 + w2 + 32) : Math.round(w1 + 24);
        let legendH = 28;
        let legendX = w - legendW - 16;
        let legendY = h - 38;

        // 動的キリ番スケール（Nice Scale）の算出：画面上で約80〜120pxに収まる最適な目盛長を決定
        let barX = 60;
        let barY = h - 25;
        let targetBarPx = 100;
        let niceUnit = (algoVal === 'ML') ? 0.1 : 10;
        let barLength = targetBarPx;
        if (scaleX > 0) {
            let rawUnit = targetBarPx / scaleX;
            let exponent = Math.floor(Math.log10(rawUnit));
            let fraction = rawUnit / Math.pow(10, exponent);
            let niceFraction;
            if (fraction < 1.5) {
                niceFraction = 1;
            ...
            niceUnit = niceFraction * Math.pow(10, exponent);
            if (exponent < 0) {
                niceUnit = parseFloat(niceUnit.toFixed(Math.abs(exponent) + 2));
            } else {
                niceUnit = Math.round(niceUnit);
            }
            barLength = niceUnit * scaleX;
            // 凡例ボックスに届かないよう最大幅ガード（最低40pxのクリアランスを確保）
            let maxBarLength = Math.max(30, legendX - barX - 40);
            if (barLength > maxBarLength) {
                barLength = maxBarLength;
            }
        }

        let unitName = algoVal === 'MP' ? 'Mutations' : (algoVal === 'ML' ? 'Substitutions/Site' : 'Mismatch');
        let barLabel = `${niceUnit} ${unitName}`;
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'alphabetic';
        ctx.fillText(barLabel, barX + (barLength / 2), barY - 7);
        ...
        if (hasBootstrap) {
            ctx.textAlign = 'left';
            ctx.fillStyle = '#2c3e50';
            ctx.fillText(branchLabel, legendX + 10, legendY + legendH / 2);

            ctx.fillStyle = '#16a085';
            ctx.fillText('% Bootstrap', legendX + 10 + w1 + 12, legendY + legendH / 2);
        } else {
            ctx.textAlign = 'center';
            ctx.fillStyle = '#2c3e50';
            ctx.fillText(branchLabel, legendX + legendW / 2, legendY + legendH / 2);
        }
    }
    ...
    window.addEventListener('load', () => {
        if (window.isPhyloHydrating) return;
        const raw = sessionStorage.getItem(PHYLO_WORKSPACE_KEY);
        if (raw) {
            try {
                const d = JSON.parse(raw);
                if (d && d.hasCalculated) return;
            } catch(e) {}
        }
        updatePhyloExportButtonsState(false);
    });
    if (!sessionStorage.getItem(PHYLO_WORKSPACE_KEY)) {
        updatePhyloExportButtonsState(false);
    }
    ...
    function isPhyloTreeCompleted() {
        ...
        const activeClusters = clusters.filter(c => c && (c.active || c.active === undefined));
        if (activeClusters.length === 1 && activeClusters[0].left !== null && activeClusters[0].right !== null) return true;
        if (currentTreeRoot && currentTreeRoot.left !== null && currentTreeRoot.right !== null) return true;
        return false;
    }
    ...
    function restorePhyloWorkspace() {
        ...
        try {
            ...
            if (data.hasCalculated && data.clusters && data.clusters.length > 0) {
                ...
                currentTreeRoot = clusters.find(c => c && c.active) || clusters[0];
                ...
                if (data.analysisLogs && data.analysisLogs.length > 0) {
                    ...
                    analysisLogs.forEach(log => {
                        if (log && log.snapshot && log.snapshot.treeRoot) {
                            log.snapshot.treeRoot.active = true;
                            if (typeof restoreParentLinks === 'function') {
                                restoreParentLinks(log.snapshot.treeRoot);
                            }
                        }
                    });
                }
                ...
                if (data.isHeatmapActive !== undefined) {
                    isHeatmapActive = data.isHeatmapActive;
                    const btnTH = document.getElementById('btnToggleHeatmap');
                    if (btnTH) {
                        btnTH.disabled = false;
                        if (isHeatmapActive) {
                            btnTH.classList.add('btn-heatmap-active');
                            btnTH.innerHTML = 'ヒートマップ';
                        } else {
                            btnTH.classList.remove('btn-heatmap-active');
                            btnTH.innerHTML = 'ヒートマップ';
                        }
                    }
                } else {
...
// [Bio-Edu Suite v36.2] Phylo Cloud Hydration Listener
window.restorePhyloWorkspace = restorePhyloWorkspace;
window.savePhyloWorkspace = savePhyloWorkspace;

window.addEventListener('bio_edu_cloud_synced', () => {
    if (typeof restorePhyloWorkspace === 'function') {
        restorePhyloWorkspace();
    }
});
window.addEventListener('bio_edu_preset_loaded', () => {
    if (typeof restorePhyloWorkspace === 'function') {
        restorePhyloWorkspace();
    }
});
