...
// --- ツールチップ機能 (ガイドライン第6項④ 準拠) ---
    ...
    function isPhyloTreeCompleted() {
        ...
        // アクティブなクラスタがちょうど1つ（根ノード）であり、かつ子ノード（枝分かれ）を持っていること
    ...
    function savePhyloWorkspace() {
        ...
        // 【安全弁】すでに完成データが保存されている場合、初期化時（isResetting）以外は未完了データによる上書き破壊を物理遮断
        ...
        try {
            // ★超重要: parentポインタを除外して循環参照エラー(Converting circular structure to JSON)を完全に回避
    ...
    function restorePhyloWorkspace() {
        ...
        try {
            ...
            if (data.hasCalculated && data.clusters && data.clusters.length > 0) {
                ...
                // ★最重要: 本物の木構造データからCanvasへ直接描画（タイミング問題を完全解消）
                drawTree();
