    ...
    function handleExecute() {
        ...
        try {
            ...
            if (!result.success) {
                ...
                if (btnExecute) { btnExecute.disabled = false; btnExecute.innerText = '▶ 実行'; }
                return;
            }
        ...
        worker.onmessage = function(e) {
            if (e.data.type === 'PROGRESS_EVAL') {
                ...
                if(el) {
                    ...
                    document.getElementById('statusLog').innerHTML = `[SYSTEM] Web Workerにて最大節約法（総当たり探索）を実行中... (<span id="evalProgress" style="color:#e67e22; font-weight:bold;">${e.data.completed} / ${e.data.total}</span> パターン評価済み)`;
                }
        ...
        worker.onmessage = function(e) {
            if (e.data.type === 'PROGRESS_EVAL') {
                ...
                if(el) {
                    ...
                    document.getElementById('statusLog').innerHTML = `[SYSTEM] Web Workerにて最尤法（総当たり探索）を実行中... (<span id="evalProgress" style="color:#e67e22; font-weight:bold;">${e.data.completed} / ${e.data.total}</span> パターン評価済み)`;
                }
            } else if (e.data.type === 'SUCCESS') {
                const rootNode = e.data.tree;
                restoreParentLinks(rootNode);
