// --- ツールチップ機能 (ガイドライン第6項④ 準拠) ---
    ...
    function switchAlgo() {
        ...
        if (algo === 'UPGMA' || algo === 'NJ') {
            if (ratMatrix) ratMatrix.innerHTML = '<strong>【根拠：距離行列とは？】</strong><br>解析ソフトは、まず「AとBは何文字違うか？」という総当たりで比較を行い、配列の違いの割合を表にした<strong>「距離行列」</strong>を作ります。共通祖先から分岐してからの時間が長いほど、塩基配列やアミノ酸配列の違い（距離）は大きくなるという前提です。';
    ...
    function executeMP(fastaText, result) {
        ...
        document.getElementById('matrixContainer').innerHTML = `
            <div style="padding:20px; text-align:center; font-family: monospace;">
                ...
                <div style="display: flex; justify-content: center; gap: 20px; font-size: 13px; color: #34495e; margin-bottom: 15px;">
                    <div>経過時間: <span id="mpElapsedTime" style="font-weight: bold; color: #2980b9;">00:00.0</span></div>
                    <div>評価中パターン: <span id="mpEvalCount" style="font-weight: bold; color: #e67e22;">0 / ?</span></div>
                    <div>ETA: <span id="mpEtaTime" style="font-weight: bold; color: #27ae60;">計算中...</span></div>
                </div>
        ...
        let timerInterval = setInterval(() => {
            ...
            let elTimeSpan = document.getElementById('mpElapsedTime');
            if(elTimeSpan) elTimeSpan.innerText = `${m}:${s}.${ms}`;
        }, 100);

        worker.onmessage = function(e) {
            if (e.data.type === 'PROGRESS_EVAL') {
                let evalCountSpan = document.getElementById('mpEvalCount');
                let etaSpan = document.getElementById('mpEtaTime');
                if (evalCountSpan) {
                    evalCountSpan.innerText = `${e.data.completed} / ${e.data.total}`;
                }
                if (etaSpan && e.data.completed > 0) {
                    let elapsedSec = (Date.now() - startTime) / 1000;
                    let remaining = e.data.total - e.data.completed;
                    let etaSec = (elapsedSec / e.data.completed) * remaining;
                    etaSpan.innerText = `約 ${etaSec.toFixed(1)} 秒`;
                }
                ...
                if (bsVal > 0) {
                    ...
                    let bsStartTime = Date.now();

                    bsWorker.onmessage = function(bsEvent) {
                        if (bsEvent.data.type === 'PROGRESS') {
                            let elapsedSec = (Date.now() - bsStartTime) / 1000;
                            let remaining = bsEvent.data.total - bsEvent.data.completed;
                            let etaSec = (elapsedSec / bsEvent.data.completed) * remaining;
                            
                            document.getElementById('statusLog').innerHTML = `ブートストラップ解析を実行中... (<span style="color:#e67e22; font-weight:bold;">${bsEvent.data.completed}</span>/${bsVal} 回完了) | 経過時間: ${elapsedSec.toFixed(1)}秒 | 残り約: ${etaSec.toFixed(1)}秒`;
    ...
    function executeML(fastaText, result) {
        ...
        document.getElementById('matrixContainer').innerHTML = `
            <div style="padding:20px; text-align:center; font-family: monospace;">
                ...
                <div style="display: flex; justify-content: center; gap: 20px; font-size: 13px; color: #34495e; margin-bottom: 15px;">
                    <div>経過時間: <span id="mlElapsedTime" style="font-weight: bold; color: #2980b9;">00:00.0</span></div>
                    <div>評価中パターン: <span id="mlEvalCount" style="font-weight: bold; color: #e67e22;">0 / ?</span></div>
                    <div>ETA: <span id="mlEtaTime" style="font-weight: bold; color: #27ae60;">計算中...</span></div>
                </div>
                <div style="margin-top:15px; width:100%; max-width: 400px; margin-left: auto; margin-right: auto; background:#ecf0f1; height:8px; border-radius:4px; overflow:hidden; position:relative;">
                    <div style="position:absolute; top:0; left:0; height:100%; width:30%; background:#3498db; animation: loading-mp 1.5s infinite ease-in-out;"></div>
                </div>
        ...
        let timerInterval = setInterval(() => {
            ...
            let m = String(Math.floor(totalSeconds / 60)).padStart(2, '0');
            let s = String(totalSeconds % 60).padStart(2, '0');
            let elTimeSpan = document.getElementById('mlElapsedTime');
            if(elTimeSpan) elTimeSpan.innerText = `${m}:${s}.${ms}`;
        }, 100);

        worker.onmessage = function(e) {
            if (e.data.type === 'PROGRESS_EVAL') {
                let evalCountSpan = document.getElementById('mlEvalCount');
                let etaSpan = document.getElementById('mlEtaTime');
                if (evalCountSpan) {
                    evalCountSpan.innerText = `${e.data.completed} / ${e.data.total}`;
                }
                if (etaSpan && e.data.completed > 0) {
            ...
            if (bsVal > 0) {
                ...
                worker.onmessage = function(e) {
                    if (e.data.type === 'PROGRESS') {
                        ...
                        document.getElementById('statusLog').innerHTML = `ブートストラップ解析を実行中... (<span style="color:#e67e22; font-weight:bold;">${e.data.completed}</span>/${bsVal} 回完了) | 経過時間: ${elapsedSec.toFixed(1)}秒 | 残り約: ${etaSec.toFixed(1)}秒`;
