// --- ツールチップ機能 (ガイドライン第6項④ 準拠) ---
    ...
    function switchAlgo() {
        ...
        function setBsOptionState(radioEl, isDisabled) {
            if (!radioEl) return;
            radioEl.disabled = isDisabled;
            const label = radioEl.closest('label') || document.querySelector(`label[for="${radioEl.id}"]`);
            if (label) {
                if (isDisabled) {
        ...
        if (algo === 'UPGMA' || algo === 'NJ') {
            setBsOptionState(bs100, false);
            setBsOptionState(bs1000, false);
        } else if (algo === 'MP') {
            setBsOptionState(bs100, false);
            setBsOptionState(bs1000, true);
            if (bs1000 && bs1000.checked) {
                if (bsNone) bsNone.checked = true;
            }
            showToast("※最大節約法(MP法)は計算負荷が高いため、ブートストラップは最大100回までです", "info");
        } else if (algo === 'ML') {
            setBsOptionState(bs100, true);
            setBsOptionState(bs1000, true);
            if ((bs100 && bs100.checked) || (bs1000 && bs1000.checked)) {
                if (bsNone) bsNone.checked = true;
            }
    ...
    function toggleUIState(isProcessing) {
        ...
        if (isProcessing) {
            ...
            if (bsRadios) {
                bsRadios.forEach(r => setBsOptionState(r, false)); // 一旦すべて解除
            }
            if (algo === 'MP') {
                if (bs1000) setBsOptionState(bs1000, true); // MP法は1000回を禁止
            } else if (algo === 'ML') {
                if (bs100) setBsOptionState(bs100, true); // ML法は100回を禁止
                if (bs1000) setBsOptionState(bs1000, true); // ML法は1000回を禁止
            }
        }
    }
