...
// --- ツールチップ機能 (ガイドライン第6項④ 準拠) ---
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('.sidebar .nav-item, .sidebar-header a').forEach(link => {
            link.addEventListener('click', (e) => {
                // ★画面遷移の直前に現在の状態を確実に保存し、データ消失を物理遮断
                ...
                if (window.isPhyloCalculating) {
                    ...
                    if (typeof showConfirm === 'function') {
                        showConfirm("現在、系統樹の解析計算を実行中です。中断して別のアプリへ移動しますか？", () => {
                            window.isPhyloCalculating = false;
                            sessionStorage.removeItem(PHYLO_WORKSPACE_KEY); // ★古いキー名を修正
    ...
    // ==========================================
    // 同一セッション内ステート保持＆安全ハイドレーション規格 (v35.1 準拠)
    // ==========================================
    ...
    function savePhyloWorkspace() {
        ...
        const hasCalc = clusters && clusters.length > 0 && clusters.some(c => c.active);

        // 【安全弁】すでに完成データが保存されている場合、手動初期化時（isResetting）以外は空データでの上書き破壊を禁止
        const existingRaw = sessionStorage.getItem(PHYLO_WORKSPACE_KEY);
        if (existingRaw && !hasCalc && !window.isResetting) {
            try {
        ...
        const stateData = {
            ...
            hasCalculated: hasCalc,
            clusters: hasCalc ? clusters : null,
            originalNames: hasCalc ? originalNames : null,
            originalSeqs: hasCalc ? originalSeqs : null,
            distanceMatrix: hasCalc ? distanceMatrix : null,
            initialDistanceMatrix: hasCalc ? initialDistanceMatrix : null,
            statusLogHtml: statusLog ? statusLog.innerHTML : '',
            analysisLogs: typeof analysisLogs !== 'undefined' ? analysisLogs : [], // ★追加
            logTableHtml: document.getElementById('logTableBody') ? document.getElementById('logTableBody').innerHTML : '', // ★追加
            newick: newickStr,
            stepCount: stepCount,
            timestamp: Date.now()
        };
        ...
        try {
            // parent参照を除外して循環参照エラーを物理遮断
            const json = JSON.stringify(stateData, (key, value) => {
                if (key === 'parent') return undefined;
                return value;
            });
            sessionStorage.setItem(PHYLO_WORKSPACE_KEY, json);
        } catch (e) {
            console.warn("セッション保存に失敗しました:", e);
        }
    }

    // 2. バリデーション先行型ハイドレーション (復元＆寸止め安全機構)
    function restorePhyloWorkspace() {
        ...
        // フェイルセーフタイマー：万が一解除イベントが消失しても500ms後には確実に強制解除
        setTimeout(() => {
            window.isPhyloHydrating = false;
        }, 500);
        ...
        try {
            ...
            // UIロックが残っていた場合は強制解除（フリーズ防止フェイルセーフ）
            toggleUIState(false);

            // ① 各フォーム要素の入力・選択値を完全復元 (操作性の担保)
            if (data.algo) {
                const algoSelect = document.getElementById('algoSelect');
                if (algoSelect) {
                    algoSelect.value = data.algo;
                    // 復元時はデータをクリアしないよう、アルゴリズム切替時のUI制御のみを手動で同期
                    const bs100 = document.getElementById('bs_100');
                    const bs1000 = document.getElementById('bs_1000');
                    if (bs100 && bs1000) {
                        bs100.disabled = false;
            ...
            const fastaText = data.fasta ? data.fasta.trim() : "";
            if (!fastaText) {
                window.isPhyloHydrating = false;
                return;
            }

            // ② 【関所】計算投入前のバリデーション検査 (安全性の担保・寸止め)
            const valResult = PhyloEngine.parseAndCalculateMatrix(fastaText);
            
            if (!valResult.success) {
                // 不正ステート時は再計算を寸止めし、UIを固めずに警告パネルを表示して待機
                toggleUIState(false);
                if (valResult.error === 'LENGTH_MISMATCH') {
                    const modal = document.getElementById('errorSuggestModal');
                    if (modal) modal.style.display = 'flex';
                    document.getElementById('statusLog').innerHTML = 
                        '<span style="color:var(--danger); font-weight:bold;">[復元待機] 塩基配列/アミノ酸配列の長さが揃っていません。テキストエリア内の配列を修正してください。</span>';
                } else {
                    showToast(valResult.message || "配列データに不備があります", "warning");
                    document.getElementById('statusLog').innerHTML = 
                        `<span style="color:var(--danger); font-weight:bold;">[復元待機] ${valResult.message || "データ不備"}のため自動再計算を一時停止しました。</span>`;
                }
                window.isPhyloHydrating = false;
                return;
            }
            ...
            // ③ バリデーション通過時：
            // すでに計算完了済みのNewickテキストや距離行列が sessionStorage に保存されている場合は、
            // それを読み込んでCanvasへ静的に再描画・ボタン活性化のみを行い、
            // handleExecute() によるゼロからの再解析・ステータスリセットが走らないようループ遮断
            if (data.hasCalculated && data.clusters && data.clusters.length > 0) {
                ...
                originalNames = data.originalNames || valResult.names || [];
                originalSeqs = data.originalSeqs || valResult.seqs || [];
                distanceMatrix = data.distanceMatrix || valResult.distanceMatrix || [];
                initialDistanceMatrix = data.initialDistanceMatrix || (valResult.distanceMatrix ? valResult.distanceMatrix.map(row => [...row]) : []);
                ...
                if (data.statusLogHtml) {
                    const statusLog = document.getElementById('statusLog');
                    if (statusLog) statusLog.innerHTML = data.statusLogHtml;
                }
                ...
                // DOM復元による遅延イベント（switchAlgo等）が落ち着くのを待ってから描画する
                setTimeout(() => {
                    // 行列描画の復元（UPGMA / NJ）
                    if (data.algo === 'UPGMA' || data.algo === 'NJ') {
                        if (typeof renderMatrix === 'function') {
                            renderMatrix();
                            if (clusters.filter(c => c.active).length <= 1) {
                                const matrixContainer = document.getElementById('matrixContainer');
                                if (matrixContainer && !matrixContainer.innerHTML.includes('全ての分類群の結合が完了しました')) {
                                    matrixContainer.innerHTML += "<div style='text-align:center; padding:15px; color:#27ae60; font-weight:bold; font-size:14px;'>全ての分類群の結合が完了しました</div>";
                                }
                            }
                        }
                    }
                    ...
                    // Canvasへ静的に再描画（ここで確実に描画させる）
                    drawTree();
                    
                    // ★共通スクリプト等との競合によるCanvasクリアを打ち消すための二段目再描画
                    setTimeout(() => {
            ...
            console.error("ハイドレーション処理で例外が発生したため安全に初期化しました:", err);
    ...
    document.getElementById('fastaInput')?.addEventListener('input', () => {
        ...
        saveTimeout = setTimeout(savePhyloWorkspace, 200);
    });
    ...
    // 画面非表示（タブ切替・離脱）時の瞬間セーブ
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'hidden') {
            savePhyloWorkspace();
        } else if (document.visibilityState === 'visible') {
            if (!window.isPhyloCalculating && !window.isPhyloHydrating) {
                const raw = sessionStorage.getItem(PHYLO_WORKSPACE_KEY);
                if (raw) {
                    try {
                        const d = JSON.parse(raw);
                        if (d && d.hasCalculated && clusters && clusters.length > 0 && clusters.some(c => c.active)) {
                            return;
                        }
                    } catch(e) {}
                }
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        }, 150); // 150msの遅延を入れて共通スクリプトの処理後を狙う
    });
