// --- ツールチップ機能 (ガイドライン第6項④ 準拠) ---
    ...
    function changeOutgroup() {
        drawTree();
        if (!isHydrating && !window.isPhyloHydrating) {
            savePhyloWorkspace();
        }
    }
    ...
    document.getElementById('fastaInput')?.addEventListener('input', () => {
        if (isHydrating || window.isPhyloHydrating) return;
        updatePhyloExportButtonsState(false);
    });
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'hidden') {
            ...
            if (!window.isPhyloCalculating && !isHydrating && !window.isPhyloHydrating) {
                const raw = sessionStorage.getItem(PHYLO_WORKSPACE_KEY);
                if (raw) {
                    try {
                        const d = JSON.parse(raw);
