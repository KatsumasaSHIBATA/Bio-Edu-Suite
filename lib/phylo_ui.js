// --- ツールチップ機能 (ガイドライン第6項④ 準拠) ---
    ...
    function executeMP(fastaText, result) {
        ...
        document.getElementById('statusLog').innerHTML = `[SYSTEM] Web Workerにて最大節約法（総当たり探索）を実行中...`;
        ...
        worker.onmessage = function(e) {
            if (e.data.type === 'SUCCESS') {
                ...
                if (bsVal > 0) {
                    document.getElementById('statusLog').innerHTML = `ブートストラップ解析(${bsVal}回)をWeb Workerで実行中... (0/${bsVal} 回完了)`;
                    document.getElementById('btnSaveImg').disabled = true;
                    document.getElementById('btnCopyNewick').disabled = true;
                    ...
                    bsWorker.onmessage = function(bsEvent) {
                        if (bsEvent.data.type === 'PROGRESS') {
                            document.getElementById('statusLog').innerHTML = `ブートストラップ解析(${bsVal}回)をWeb Workerで実行中... (<span style="color:#e67e22; font-weight:bold;">${bsEvent.data.completed}</span>/${bsVal} 回完了)`;
                            ...
                            document.getElementById('btnSaveImg').disabled = false;
                            document.getElementById('btnCopyNewick').disabled = false;
    ...
    function executeML(fastaText, result) {
        ...
        document.getElementById('statusLog').innerHTML = `[SYSTEM] Web Workerにて最尤法（総当たり探索）を実行中...`;
    ...
    function renderInteractiveSVG(treeType, site) {
        ...
        if (treeType === 1) { 
            points = [
                {x: 50, y: 30, b: site.bases[0], label: mpNames[0]}, 
                {x: 110, y: 30, b: site.bases[1], label: mpNames[1]}, 
                {x: 170, y: 30, b: site.bases[2], label: mpNames[2]}, 
                {x: 230, y: 30, b: site.bases[3], label: mpNames[3]} 
            ];
        } else if (treeType === 2) { 
            points = [
                {x: 50, y: 30, b: site.bases[0], label: mpNames[0]}, 
                {x: 110, y: 30, b: site.bases[2], label: mpNames[2]}, 
                {x: 170, y: 30, b: site.bases[1], label: mpNames[1]}, 
                {x: 230, y: 30, b: site.bases[3], label: mpNames[3]} 
            ];
        } else { 
            points = [
                {x: 50, y: 30, b: site.bases[0], label: mpNames[0]}, 
                {x: 110, y: 30, b: site.bases[3], label: mpNames[3]}, 
                {x: 170, y: 30, b: site.bases[1], label: mpNames[1]}, 
                {x: 230, y: 30, b: site.bases[2], label: mpNames[2]} 
            ];
        }
    ...
    function revealStep2Answers() {
        let site = allInformativeSites[activeMPInteractiveSiteIndex];
        for (let t = 1; t <= 3; t++) {
            let foundCombination = site.optimalPlacements[t - 1];

            if (foundCombination) {
                activePlacedMutations[t - 1] = foundCombination;
                for (let b = 0; b < 5; b++) {
                    const line = document.getElementById(`line_${t}_${b}`);
                    const starGroup = document.getElementById(`star_${t}_${b}`);
                    if (foundCombination[b]) {
                        line.classList.add('branch-mutated');
                        starGroup.setAttribute('opacity', '1');
                    } else {
                        line.classList.remove('branch-mutated');
        ...
        updateMutationCounters();
        verifyPlacedMutations();
        showToast("最小突然変異配置の正解を表示しました。");
    }
    ...
    function drawMPTreeCanvas(canvasId, treeType) {
        ...
        const root = {x: 120, y: 110};
        const n1 = {x: 150, y: 80};
        const n2 = {x: 180, y: 50};
        
        let points = [];
        if (treeType === 1) { 
            points = [ {x: 30, y: 20, label: mpNames[0]}, {x: 90, y: 20, label: mpNames[1]}, {x: 150, y: 20, label: mpNames[2]}, {x: 210, y: 20, label: mpNames[3]} ];
        } else if (treeType === 2) { 
            points = [ {x: 30, y: 20, label: mpNames[0]}, {x: 90, y: 20, label: mpNames[2]}, {x: 150, y: 20, label: mpNames[1]}, {x: 210, y: 20, label: mpNames[3]} ];
        } else {
            points = [ {x: 30, y: 20, label: mpNames[0]}, {x: 90, y: 20, label: mpNames[3]}, {x: 150, y: 20, label: mpNames[1]}, {x: 210, y: 20, label: mpNames[2]} ];
        }
    ...
    function mlCheckMutationType(userChoice) {
        ...
        if (isCorrect) {
            if(isTrans) {
                feedback.innerHTML = `<span style="color:var(--success);">大正解！</span>化学的性質が似た者同士（プリン同士 または ピリミジン同士）の変化なので、これは<strong>「遷移 (Transition)」</strong>です。<br><span style="color:#7f8c8d; font-weight:normal;">DNA構造に無理が生じないため、自然界ではこの変化が圧倒的に起こりやすいです。</span>`;
            } else {
                feedback.innerHTML = `<span style="color:var(--success);">大正解！</span>プリン塩基とピリミジン塩基をまたぐ変化なので、これは<strong>「置換 (Transversion)」</strong>です。<br><span style="color:#7f8c8d; font-weight:normal;">※重要用語規格においてトランスバージョンは「置換（塩基置換）」と訳されます。</span>`;
            }
    ...
    function drawMLInteractiveTree(treeType, site) {
        ...
        const points = [
            {x: 50, y: 30, label: mlNames[pIdx[0]], b: site.bases[pIdx[0]]}, 
            {x: 110, y: 30, label: mlNames[pIdx[1]], b: site.bases[pIdx[1]]}, 
            {x: 170, y: 30, label: mlNames[pIdx[2]], b: site.bases[pIdx[2]]}, 
            {x: 230, y: 30, label: mlNames[pIdx[3]], b: site.bases[pIdx[3]]}
        ];
    ...
    function selectMLTree(index) {
        const isCorrect = (index === mlSelectedTreeType);
        const model = document.querySelector('input[name="ml_sim_model"]:checked').value;
        ...
        for(let i=1; i<=3; i++) {
            let card = document.getElementById(`mlCard${i}`);
            if (i === index) {
                card.style.borderColor = isCorrect ? '#27ae60' : '#e74c3c';
                card.style.boxShadow = isCorrect ? '0 0 0 2px #27ae60' : '0 0 0 2px #e74c3c';
            } else {
                card.style.borderColor = 'var(--border-color)';
                card.style.boxShadow = 'none';
            }
        ...
        if (isCorrect) {
            ...
            let exp = "";
            if (model === 'K80') {
                exp = `K80モデルを用いたことで「遷移」の発生確率が高く評価され、他の樹形と比べて圧倒的に高い尤度スコアを叩き出しました。実際の研究でも、適切な進化モデルを選ぶことが真実の系統樹に辿り着くためのカギになります。`;
            } else {
                exp = `JC69モデルでも尤度が一番高くなりましたね。さらに「K80モデル」に切り替えてみると、スコアに劇的な変化が起きるはずです。試してみてください。`;
            }

            feedback.innerHTML = `
                <div class="assist-item">
                    <div class="assist-title" style="color:#27ae60; font-weight:bold; margin-bottom:5px;">正解！最も尤度（起こりやすさ）が高い樹形を選択しました。</div>
    ...
    function proceedToMLStep3() {
        ...
        document.getElementById('mlStep3Feedback').style.display = 'none';
            ...
            if (bsVal > 0) {
                ...
                worker.onmessage = function(e) {
                    if (e.data.type === 'PROGRESS') {
                        document.getElementById('statusLog').innerHTML = `ブートストラップ解析(${bsVal}回)をWeb Workerで実行中... (<span style="color:#e67e22; font-weight:bold;">${e.data.completed}</span>/${bsVal} 回完了)`;
    ...
    function drawTree() {
        ...
        ctx.beginPath();
        ctx.moveTo(legendX + 96, legendY + 5);
        ctx.lineTo(legendX + 96, legendY + legendH - 5);
        ctx.strokeStyle = '#bdc3c7';
        ctx.stroke();
