// --- ツールチップ機能 (ガイドライン第6項④ 準拠) ---
    ...
    function savePhyloWorkspace() {
        ...
        const stateData = {
            ...
            hasCalculated: clusters && clusters.length > 0 && clusters.some(c => c.active),
            timestamp: Date.now()
        };
    ...
    function restorePhyloWorkspace() {
        ...
        try {
            ...
            // ③ バリデーション通過時のみ、直前が計算完了状態だった場合に安全に再描画
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'hidden') {
            // 計算中フラグが立っていた場合でも復帰時に固まらないようセーフティセーブ
            savePhyloWorkspace();
        } else if (document.visibilityState === 'visible') {
            // タブ復帰時：UIロックを解除し、寸止めチェックを走らせる
            if (!window.isPhyloCalculating) {
