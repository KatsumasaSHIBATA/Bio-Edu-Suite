...
self.onmessage = function(e) {
    if (e.data.type === 'CALC_MP') {
        ...
        let evalCount = 0;
        let totalTopologies = topologies.length;
        let reportInterval = 10; // 常に10パターンごとに報告してUIをパラパラ動かす
        ...
        for (let tree of topologies) {
            ...
            evalCount++;
            if (evalCount % reportInterval === 0 || evalCount === totalTopologies) {
                self.postMessage({ type: 'PROGRESS_EVAL', completed: evalCount, total: totalTopologies });
            }
        }
    ...
    if (e.data.type === 'CALC_ML') {
        ...
        let evalCount = 0;
        let totalTopologies = topologies.length;
        let reportInterval = 1; // 常に1パターンごとに報告してUIをパラパラ動かす
