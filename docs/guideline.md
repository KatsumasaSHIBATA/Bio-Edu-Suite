# **🎨 Bio-Edu Suite 統合UI/UX・開発ガイドライン (v36.5_2026-09-23:最新版)**
...
## **【改訂・サルベージ履歴】**
* **v36.5 (2026-09-23):** 解析ログ・タイムマシン復元規格、テーブル行ボーダーレス・ハイライト規格、およびログ俯瞰エリア拡張規格の法制化。
  - [テーブル行アクティブ・ハイライト規格]: 選択行（.active-log）において、セル境界の縦線や上下線の段差ノイズを物理遮断するため、ボーダーを全廃しフラットな背景色面（#d1f2eb）のみで選択状態を示すデザイン規格を制定。
  - [解析ログ・タイムマシン完全スナップショット保持規格]: ログ配列（analysisLogs）内に系統樹根ノード、距離行列、アルゴリズム、外群の完全なスナップショットをディープコピーして保持し、クリック時に描画エンジンおよびエクスポート用データ（Newick/TSV）を過去ステートへ完全再充填する機構を義務化。
  - [比較ログテーブル表示領域規格]: 複数モデルの試行結果を快適に比較・選択できるよう、ログテーブルの表示ラッパーに max-height: 380px を確保し、スクロール不要で3〜5件以上を俯瞰できる情報一覧性を標準化。
* **v36.4 (2026-09-22):** 教員・生徒ハイブリッド巻き戻し防止機構（Anti-Rollback Guard）およびダッシュボード教材サンプル自律復元規格の法制化。
  - [Anti-Rollback Guard（ステート巻き戻し物理遮断）]: 同一セッション内に有効な作業データ（bio_edu_workspace_*, bio_edu_ws_*, bio_edu_draft_*）が存在する場合、アプリ間遷移時のクラウド同期（ダウンロード）による過去スナップショットでの巻き戻し・上書き・ゾンビ復活を物理遮断し、常にローカル最新をクラウドへ同期保存して早期リターンする調停エンジンを義務化。
  - [入室直後調停フラグ (bio_edu_just_joined)]: joinRoom 実行時の初回入室直後のみクラウドからのダウンロード展開を優先し、入室完了後のアプリ間遷移ではローカル最新を優先するライフサイクル規約を制定。
  - [ダッシュボード初期サンプル復元導線]: ストレージ未保存時のダーウィンフィンチ4種自動復元と、空状態（.empty-state）におけるワンクリック手動復元ボタン（restoreDefaultSamples）の常時配備を義務化。
...
* **v36.1 (2026-09-20):** 検証後ネクストアクション3分岐規格（Post-Verification 3-Way Action Standard）およびモデル・チャット継続規律の法制化。
  ...
  - [モデル維持・チャット継続・リセット規律]: Web側司令塔は常に Pro、Cline現場は常に Flash-Lite を維持し切り替えない。Case B（リカバリー）時はWeb側・Cline側ともにチャットを「継続」して文脈を保持し、Case A（完了）およびCase C（白紙撤回）時はチャットを「Clear（新規作成）」して過去コンテキストを遮断する。


* **v36.1 (2026-09-20):** 検証後ネクストアクション3分岐規格（Post-Verification 3-Way Action Standard）の法制化。
  - [3分岐アクションの標準化]: 司令塔が出力するすべての実装・改修指示書の末尾に、動作確認後のステート遷移（Case A: 完全動作時のCommit & Push、Case B: 不具合時の生ログ丸ごとリカバリー、Case C: 致命的崩壊時のRollback白紙撤回）の明示を義務化。
  - [Web側生ログ丸ごと受け入れ特例の制定]: ユーザーの手作業フリクションを根絶するため、不具合発生時におけるスタックトレースやエラー箇所の切り抜き作業を不要とし、Web版Geminiへのエラーログ・Cline報告書の全文コピペ／添付を正式許可。司令塔側が自律的に根本原因を特定・蒸留する。



* **v36.0 (2026-09-19):** ダイアレル交配分析（Diallel Analysis）およびG×Eモデル拡張規格の法制化。
  - [Griffing Method 2規格]: 親系統（自殖）およびF1（片側交配）を含むグリフィング第2法・固定効果モデル（Model I）を標準数理モデルとして策定。
  - [組合せ能力と遺伝効果の定義]: 一般組合せ能力（GCA＝相加的遺伝効果）および特定組合せ能力（SCA＝顕性効果・非相加的相互作用）の計算式・ANOVA分散分析表の出力要件を規定。
  - [用語規格の絶対遵守]: 「優性効果」を完全排除し、「顕性効果（dominance effect）」「顕性分散」「相加的効果」の表記を義務付け。
  - [アプリ⑫コンテキスト分離]: アプリ⑫（Statistical Genetics Lab）における「G×E 相互作用解析（AMMI/反応規範）」と「ダイアレル交配分析（Diallel）」のタブ切替による完全分離を義務化。

* **v35.9 (2026-09-19):** マルチデータセットのサンプルIDベース内部結合（Inner Join）および欠損自動除外規格の法制化。
  - [クロスデータ・バリデーション]: アプリ⑪等の統合解析ツールにおいて、遺伝データ（Mutation）と形態データ（Variation）を統合する際は、暗黙的な順序並びではなく、必ず共通の Sample_ID に基づく内部結合（Inner Join）を行ってから行列計算・相関解析に回すこと。
  - [欠損個体の安全除外]: 片方のデータセットにしか存在しない個体・サンプルは、エラーで停止させず安全に除外してトースト警告を提示する仕様を標準化。
* **v35.8 (2026-09-19):** ワークスペース自律クリーンアップ規格（Workspace Self-Cleaning Standard）の法制化。
  - [プロジェクト肥大化・残骸蓄積の物理遮断]: OS一時ファイル（.DS_Store等）や、バッチ改修で使い捨てられたワンショットスクリプトの滞留を防止するため、スクリプト実行時・改修完了時の自律クリーンアップ（`cleanup_audit.py --apply`）をシステム規格として義務化。
* **v35.7 (2026-09-19):** 教員プリセット配信データスキーマおよび自動ハイドレーション・イベント規格の法制化。
  - [master_tasks スキーマの確定]: Firestore の `master_tasks/{taskCode}` に格納するペイロード形式（sequence, sessionData, appName, registeredAt）を規格化。
  - [カスタムイベント連携]: 生徒端末でのプリセット読込時、sessionStorage へのマッピング復元とフォームへの値注入に加え、カスタムイベント 'bio_edu_preset_loaded' をディスパッチしてサイレント再計算・描画更新を確実にトリガーする規約を明文化。
* **v35.6 (2026-09-16):** 全11大・トークン極限圧縮＆ハルシネーション物理防止アーキテクチャ規格の法制化。
  - [極限圧縮・防壁規格の制定]: APIコストの劇的削減とAIコンテキストウィンドウの純度保持を両立するため、第17項として「11大・トークン極限圧縮＆コンテキスト保護アーキテクチャ規格」を新設。
  - [AST関数抽出・骨格化の義務付け]: 巨大コードの直接読み込みを排し、ASTパーサーによる関数スコープ抽出およびクラス/関数インターフェース骨格化を標準化。
  - [巨大データの強制トランケーション]: FASTA配列や生ログの安全なトリミング（先頭・末尾＋ハッシュ長）境界を策定。
  - [ハイブリッド・ルーティング]: クラウドLLMとローカルLLM/Web Workerの計算負荷分散モデルを規定。
* **v35.5 (2026-09-10):** 隠れ教師モード（Secret Teacher Mode）および教員プリセット発行・配信規格の法制化。
  - [参加者IDによる教員ロール判定]: 参加者IDの先頭に「TEACHER」（例: `TEACHER` または `TEACHER-山田`）を入力して入室した場合、システムが自動的に教員権限（`isTeacher = true`）を判定・認識する。
  - [教員専用パブリッシュUIの動的解放]: 教員権限が有効な端末に限り、接続中モーダル内に「👑 教員専用：現在の画面データを課題プリセットとして発行」セクションを動的に表示。課題コード（例: `TASK-01`）を指定してワンクリックでFirestore（`master_tasks/{taskCode}`）へ画面上の教材データをパブリッシュ可能化。
  - [完全空箱化とゼロ・フリクション配信]: 生徒端末には読込フォームのみを提供し、教員専用UIを完全不可視化。教員が提示した課題コードを打ち込むだけで、重いファイルダウンロードや手動インポートを不要にし、授業データを一瞬で安全に展開する。
* **v35.4 (2026-09-10):** Google同期の完全廃止とKahoot方式ルーム機能（個人／班ハイブリッド同期）への一本化、および初期化多重保護・教員プリセット配信規格の法制化。
  - [認証の簡素化]: 現場のログイン摩擦をゼロにするためGoogle OAuthログインを完全廃止し、アカウント不要の「匿名認証＋ルームコード＋参加者ID（班または個人）」へ一本化。
  - [個人／班ハイブリッド]: 参加者IDに出席番号や個人名を入力することで、個人作業であっても自宅の別端末から同一コードでシームレスに作業再開を可能化。
  - [初期化の多重保護]: 端末初期化によるクラウド側データの誤破壊・連動消去を物理遮断。
  - [教員プリセット配信]: 教員が発行した課題コード（マスターデータ）を、生徒が個人・班を問わず自環境へ安全にインポートできる規格を定義。
* **v35.3 (2026-09-08):** 機能改修時の動作検証用デモデータ・QAプロトコル義務化の法制化。本番UIを侵食せず手動バケツリレーで即時検証できるよう、新機能・UI改修指示には生物学的妥当性を備えた「検証用デモデータ」と「期待動作チェックリスト」のセット出力を司令塔に義務付け。
* **v35.2 (2026-09-05):** 開発ガバナンス規格の厳格化（事前法制化の徹底、コード受領前プロンプト生成の禁止、三位一体ドキュメント監査の義務化）。
* **v35.1 (2026-09-04):** 全アプリにおける同一セッション内ステート完全保持規格（Session-Persistent Workspace Standard）の法制化。
  * **[作業空間の完全永続化]**: アプリ間を行き来しても入力データ・編集内容・解析状態が破棄されないよう、ウィンドウ生存期間中の `sessionStorage` 自動同期＆完全ハイドレーションを義務化。
  * **[データ破棄契機の厳格化]**: ウィンドウ全体の閉鎖、またはドロワー・各画面の「データを初期化」ボタン押下時のみデータをパージする仕様へ方針転換。手動バケツリレーの教育的境界（アプリ間のブラックボックス自動受け渡し禁止）は堅持。
...
* **v33.5 (2026-08-25):** クラウド連携のハイブリッド化とネイティブアプリUI/UXへの完全昇華。  
  ...
  * **\[ハイブリッド・クラウド連携\]**: 第7項にFirebase（Firestore/Auth）を用いたリアルタイム同期と、学校のネットワーク制限を回避する「完全オフライン・ファースト（ローカル自動フォールバック）」のハイブリッド設計を明文化。  
    ### 16.1 2Dデジタイザ画像計測・直感連携規格（Fluid Digitizer UX Standard）
    - **初期ステート未選択の徹底**:
      デジタイザ起動時や画像読み込み直後は、部位計測モード（A/B/C等）を未選択（グレーの `.btn-secondary`）とし、意図しない誤クリック描画を防止すること。
    - **部位ボタン押下時の自動全画面化（Zero-Click Fullscreen）**:
      画像が存在する状態で計測部位ボタン（A/B/C等）が押された場合、ボタンのアクティブ化（黄色 `.btn-primary`）と同時に、自動的に全画面ダークオーバーレイ（`fixed inset-0 z-[99999] bg-slate-950/95`）へ拡大移行させること。独立した「全画面拡大/縮小」ボタンの常時配置はUIノイズとなるため原則禁止とする。
    - **ツールバーの右寄せ配置**:
      デジタイザ上部のツールバーは、左側に「計測部位:」ラベル、右側に部位ボタン群（A/B/C）および「クリア」ボタンを右寄せ（`justify-between`）で整然と配置すること。
    - **数値インライン点灯＆ワンクリック自動復帰（Instant Copy & Restore）**:
      計測点（2点、または角度4点）のプロット完了時、ボタン枠や余計なアイコン・装飾テキストを排し、数値テキスト（例: `○○.○ px` または `○○.○°`）そのものをテーマカラー黄色（`#f1c40f`）でピカピカと発光点灯（`animate-pulse`）させること。
      この点滅数値をタップした瞬間、同期クリップボードAPIにより純粋な数値のみをクリップボードへコピーし、同時に全画面を自動縮小して元のドロワー画面へ復帰させること。
    - **全画面縮小後の発火停止**:
      通常画面に復帰した後は、数値のピカピカ点滅（発火）をピタッと停止させ、落ち着いたテキスト色（`text-slate-800`）へ戻すこと。ツールチップ案内も「クリックで数値をコピーして全画面を閉じます」から「クリックで数値をコピー（ワークシート貼付用）」へ文脈適応させること。
    - **クリアボタンによる初期化復帰**:
      全画面表示中に「クリア」が押された場合は、計測点と部位選択をリセットし、自動的に全画面を解除して通常画面へ復帰させること。
...
* **広域一括改修時のトークン制限回避プロトコル（Cline完全一本化・Pythonスクリプト駆動SOP）**: 全14アプリにまたがるCSS/HTML/JSの共通パッチ適用や広域改修において、Clineのコンテキストに全HTMLファイルを直接読み込ませて多重編集を行うことは、トークン上限の枯渇・コンテキスト破綻・コード目減りの原因となるため原則禁止とする。複数ファイルの一括更新は、「VS Code上の Cline（Actモード） ＋ Pythonスクリプト駆動型一括更新（冪等性対応）」を唯一の標準作業手順（SOP）とする。AI（Cline）に `scripts/` 配下へ多重挿入防止（冪等性）を備えた単一のPython処理スクリプト（`scripts/apply_patch.py`等）を作成させ、ターミナルで `python3 scripts/apply_patch.py` を実行して一括置換を完了させ、`git diff` および `git status` で検証する。検証完了後は、スクリプトを安全に管理・整理し、簡潔なコミットメッセージでGit保存を確定させる。

* **知見・設計合意の即時法制化原則 (Constitutional Synchronization)【v35.0 新設】**:  
  * チャット上の議論や口頭で確立された新たなアーキテクチャ設計、AIハルシネーション防止策、およびデータ連携の仕様変更を、会話履歴や作業者の記憶の中だけにとどめておくことを固く禁ずる。  
  * 新たな運用ルールや設計方針が合意された際は、実際のコード実装に着手する前に、必ず最優先で `docs/guideline.md`、`docs/workflow.md`、`docs/instruction.md`（および `.clinerules`）へ文章として明文化（法制化）し、プロジェクトの「憲法」として同期・恒久化しなければならない。ドキュメントに明文化されていない仕様に基づく開発・改修は原則無効（差し戻し対象）とする。
* **事前法制化の原則（Rule-First Deployment）【v35.2 強化】**:
  全14アプリに及ぶ共通UI変更、データ構造変更、用語規格の更新等の大規模改修は、該当コードの改修スクリプトを走らせる前に、必ず本ガイドライン（`docs/guideline.md`）へ仕様・禁止事項を明文化して確定（コミット）させなければならない。ルール無き先行実装は一切認めない。
* **コード受領前の先行プロンプト生成禁止（Anti-Rushing Protocol）【v35.2 新設】**:
  司令塔（統括アーキテクト）は、改修対象となるアプリコードの実態をユーザーから受け取り、構造を精査する前にCline実装指示書（差分パッチ指示）を発行してはならない。想像によるコード指定やフライング出力を固く禁ずる。
* **改修完了後の三位一体監査（Triad Review）【v35.2 新設】**:
  各アプリの実装・検証が完了した直後、司令塔は必ず「本ガイドライン」「開発フロー（workflow.md）」「カスタム指示書（instruction.md）」の3点について、運用のズレや新仕様の追従改変が必要かを点検し、改変案をユーザーへ提示することを義務付ける。
...
* **Phase 4 新規解析モジュール（Comparative Variant Analyzer）の規格定義**:
  * **所属フェーズ**: Phase 4（統合検証ルーム） / **テーマカラー**: `--phase-color: #8e44ad;`（高貴な紫）
  * **学術概念**: 日本学術会議（2025年版）に基づき、突然変異（mutation）と変異（variation）をクロス対比させる専用UI構造とする。
...
* **デモデータ供給ハブ（データディスパッチャー）規格**:
  * **個別アプリへのデモデータ内包禁止**: 各解析アプリ内にデモデータ呼び出しボタンや固定プリセットを配置することは固く禁ずる（第7項③空箱化の徹底）。
  * **ダッシュボードからのデータ供給**: すべての検証用データ・教材データはダッシュボード（アプリ①）の「データ生成ステーション」から発行し、メタデータ刻印を付与してクリップボードへコピーさせる。
...
* **⑦ 教師モード（Teacher / Facilitator Mode）とクラス集約規格**:
  * 一般生徒画面では「おまけUI」を一切排除し、空箱化を死守する。
  * 教師ロール（`role: "teacher"`）が有効な端末に限り、検証データの即時注入や、ダッシュボードでのクラスLIMS集約を許可する。
* **⑧ ルームコードUIノイズ抑制規格**:
  * 個別アプリの作業画面にルームコード入力フォームを常時露出させることを禁止し、ダッシュボード（アプリ①）へ一本化する。
...
### 7.4 同一セッション内ステート完全保持規格（Session-Persistent Workspace Standard）

#### ① 設計思想の転換と背景
Bio-Edu SuiteはMPA（マルチページアプリケーション）構造を採用しているため、アプリ間でデータを持ち寄る際（例: アプリ④でab1ファイルを編集し、一部をコピーしてダッシュボードに貼った後、再びアプリ④に戻る／アプリ⑧から⑩へ移動し再び⑧へ戻る等）に、元のアプリの作業内容が初期化されてしまう問題は学習体験上著しい認知負荷となる。
教育的哲学である「手動バケツリレー（アプリ間でのブラックボックスな自動データ連携の禁止）」は死守しつつ、**「同一セッション（ブラウザウィンドウ・タブを開いている間）における各アプリの個別作業空間（Workspace）は、明示的に初期化しない限り完全に維持される」** 仕様へ全面改訂する。

#### ② ステート保持・破棄の境界ルール
1. **完全維持の対象**:
   - 入力生データ（DNA/mRNA/タンパク質の塩基配列・アミノ酸配列、ab1波形パースデータ、TSV、JSON、形態測定座標等）
   - ユーザーによる編集・トリミング結果および中間生成データ
   - 全フォーム入力値（スライダー数値、ラジオボタン、チェックボックス、セレクトボックス）
   - 直近の解析・描画完了ステート（系統樹、アライメント結果、グラフ等）
2. **破棄（パージ）の唯一の契機**:
   - ブラウザのタブまたはウィンドウ全体が閉じられたとき（セッション終了）
   - グローバル・ドロワー最下部の「🗑️ データを初期化」が実行されたとき（全アプリ初期化）
   - 各アプリ画面内に配置された明示的な「初期化 / クリア」ボタンが実行されたとき（該当アプリ初期化）
3. **長時間処理（重負荷計算・外部通信）のステートパージ特例と離脱警告UI**:
   - アプリ⑤（マルチプルアラインメント）、アプリ⑦（Q BLAST）、アプリ⑧（最尤法推定）などのWeb Workerを用いた重負荷計算や外部API通信の「進行中（処理途中）」に、ユーザーが別のアプリへ遷移（離脱）しようとした場合、バックグラウンドでの計算維持は困難であるため、進行中の計算ステートは維持せず、計算開始前の状態へ差し戻す（キャンセル・リセットする）ことを義務付ける。
   - **完全維持の担保**: すでに解析・描画が完全に完了した結果ステートは、上記のパージ対象には含めず、初期化されない限り絶対に維持し続けること。
   - **警告パネル（Confirm Modal）の提示**: 計算中にグローバル・ドロワー（Peg Menu）などを経由して別アプリへの遷移を試みた際は、直ちに遷移させず、第8項で規定されたカスタム確認ダイアログ（`#confirmModal`）を動的に呼び出し、「現在処理中です。アプリを移動すると進行中の処理はキャンセルされ元の状態に戻ります。移動しますか？」という旨の警告パネルを必ず提示し、ユーザーの同意を得てから遷移（またはキャンセル）させること。

#### ③ 技術実装規格（Session Storage Workspace Engine）
全アプリにおいて、以下のキー命名規則およびライフサイクルイベントを標準実装する。

1. **ストレージキー命名規則**:
   `bio_edu_workspace_<app_name_or_phase>` （例: `bio_edu_workspace_app4`, `bio_edu_workspace_app8`）
2. **自動シリアライズ（保存）**:
   - フォーム要素（`<input>`, `<textarea>`, `<select>`）の入力変更時（`input`, `change`）に、150msのデバウンスを介して現在の作業状態をJSONシリアライズし `sessionStorage` に書き込む。
   - 画面遷移・離脱時（`pagehide`, `visibilitychange`）に、最新の描画ステートや内部フラグを含めて瞬間保存を実行する。
3. **自動ハイドレーション（復元）**:
   - `DOMContentLoaded` 時に該当キーを読み出す。
   - 保存データが存在する場合、各DOM要素へ値を復元し、初期化ガードフラグを立てた状態で `input` / `change` イベント、および解析描画関数をサイレント実行（再計算・再描画）して直前の作業状態を完全復元する。
4. **初期化処理のハンドリング**:
   - 初期化ボタン実行時は、該当する `bio_edu_workspace_*` キーを `sessionStorage.removeItem()` で削除し、初期状態（空箱）へ戻す。
   - また、リロード時のオートセーブ逆流（レースコンディション）を防ぐため、`clear()` 実行直前に `window.isResetting = true;` を宣言し、さらに `Storage.prototype.setItem = function() {};` を記述してブラウザの保存機能を一時的に無力化（物理遮断）することを義務付ける。

    4. **教員・生徒ハイブリッド巻き戻し防止機構（Anti-Rollback Guard）**:
       - アプリ間を遷移する際、ローカルの sessionStorage 内に空でない有効な作業データが存在する場合、クラウド同期関数（syncFromCloud）はクラウドからの過去スナップショットダウンロード（巻き戻し）を遮断し、ローカルの最新状態をクラウドへ確定保存（saveCurrentWorkspace）して早期リターンしなければならない。
       - これにより、ダッシュボードでサンプルを削除・編集した後に他アプリへ遷移して戻った際、過去のデータが降ってきてゾンビ復活する不具合を根本から物理遮断する。

    5. **キー正規化およびダッシュボード例外ハンドリング**:
       全アプリ共通の保存キーは `bio_edu_workspace_<app_name>` とするが、ルートアクセス（パスが空）または `index.html` の場合は動的に `bio_edu_workspace_dashboard` に正規化し、ダッシュボード固有のサンプル永続化キーとの衝突・すれ違いを防止すること。また、保存完了時は `bio_edu_request_save` カスタムイベントを送出し、バックグラウンドのクラウド同期エンジン（auth_sync.js）へ即時反映を要求すること。
...
### 7.5 Google同期の廃止とKahoot方式ルーム同期・初期化多重保護規格（v35.4 制定）

1. **Google OAuthログインの廃止とKahoot方式への完全一本化**:
   - 学校のネットワーク制限、パスワード忘れ、外部OAuth遮断による授業遅延を根絶するため、従来のGoogleアカウント認証を完全廃止する。
   - Firebase匿名認証を基盤とし、「ルームコード（例: BIO-101）」＋「参加者ID（班ID または 個人出席番号）」のみで入室・同期を行うKahoot方式へ一本化する。
   - UI上の「Googleでログイン」ボタン、アカウント表示UIを撤廃し、シンプルなルーム接続パネルへ刷新する。
   - **初回入室時ハイドレーションとローカル調停の境界**:
     未接続状態からルームコードと参加者IDを入力して入室した直後は、クラウド上に存在する最新スナップショット（他端末で作成されたデータ）のダウンロード展開を最優先とし、ローカルの初期プリセットをクラウド側データで完全にハイドレーション（更新）すること。同一セッション内でのアプリ間遷移時のみ、ローカルの最新編集状態を保護してクラウドへの巻き戻しを防ぐ調停エンジンを適用する。

2. **個人ワークとグループ協働、および自宅別端末継続の完全両立**:
   - **グループ（班）協働**: 参加者IDに班番号（例: `Team-A`）を入力することで、班員全員が同一のワークスペース（DNA/mRNA/タンパク質の塩基配列・アミノ酸配列、アライメント結果等）をリアルタイム共有する。
   - **個人ワーク＆自宅引き継ぎ**: 参加者IDに個人ID（例: `No-12`）を入力することで、自分専用の安全なクラウド領域が確保される。学校で解析した途中データを、自宅のPCやタブレットから同じ「ルームコード＋個人ID」で開くだけで、ログイン不要で即座に引き継いで解析を再開できる。

3. **教員マスターデータ（プリセット課題）の一斉配信＆安全インポート規格**:
   - 教員はダッシュボード（アプリ①）等から、当日の実習で用いる共通データ（塩基配列、変異情報、実験パラメータ）を「課題マスターコード（例: `TASK-2026A`）」として発行・提示できる。
   - 生徒は「個人ワーク」「班協働」のどちらのモードであっても、課題コードを指定して読み込むことで、教員のマスターデータを自らの作業空間へ安全に複製・インポートして解析を開始できる。

4. **初期化の多重保護規格（クラウド破壊防止）**:
   - ドロワーや各画面の「データを初期化（`confirmDataReset`）」ボタン押下時は、端末ローカルストレージ（`localStorage` / `sessionStorage`）のキャッシュクリアおよび現在ルームセッションの切断（退出）のみを実行する。
   - クラウド（Firestore）上に保存された班・個人の解析スナップショットデータに対する空データ上書き・連動消去を物理的に遮断する。
   - 誤って初期化を実行した場合でも、同一のルームコードと参加者IDを再入力することで、直前の最新解析ステートをクラウドから安全に再取得（ハイドレーション）できるように設計する。

5. **バックグラウンド・オートセーブの常時強制**:
   - アプリ間移動、タブのバックグラウンド移行（サスペンド）、および不意のブラウザ終了に備え、`visibilitychange` および `pagehide` イベント発火時に、直前の作業状態をミリ秒単位でローカルおよびクラウドへ強制保存する。

6. **隠れ教師モード（Secret Teacher Mode）と教員プリセット発行・配信規格**:
   - **判定トリガー**: 参加者ID（`participantId`）の入力値が、大文字・小文字を問わず「`TEACHER`」で始まる場合（例: `TEACHER`、`TEACHER-01`、`TEACHER-BIO` 等）、教員端末（`isTeacher = true`）として認証する。
   - **教員専用UI（シークレットパネル）**:
     - モーダル内の「接続中ビュー（`modalLoggedInView`）」において、通常生徒には非表示の「教員マスター課題データ発行パネル（`teacherPresetSection`）」を動的に解放（`display: block`）する。
     - 入力項目: 任意の英数字からなる「課題コード（例: `TASK-01`）」および「現在の画面データを登録・発行」ボタン。
   - **データ登録処理 (`registerMasterPreset`)**:
     - 実行時、現在アクティブな入力要素（`textarea`、各種フォーム）の入力値、または `sessionStorage` 内の解析ステートを収集し、Firestore の `master_tasks/{taskCode}` に `payload` として保存する。
   - **生徒端末での一斉展開 (`importMasterPreset`)**:
     - 生徒はモーダル内の課題コード入力欄にコードを入力して「読込」を押す。
     - Firestore から取得されたデータが、生徒端末の該当入力エリア（塩基配列・アミノ酸配列・パラメータ等）へ即座に流し込まれ、`input` / `change` イベントが自動発火して解析画面が展開される。
...
7. **教員プリセット配信データスキーマとイベントディスパッチ規格**:
   - **ペイロードスキーマ**: `master_tasks/{taskCode}` ドキュメントには、以下のキー構造を持つオブジェクトを格納する。
     * `sequence`: 現在アクティブな主要テキストエリア（DNA/mRNA/タンパク質の塩基配列・アミノ酸配列、計測データ等）の文字列。
     * `sessionData`: 現在の sessionStorage の全キー・バリューのスナップショット。
...
8. **入室直後調停フラグ規格（bio_edu_just_joined）**:
   - ルーム入室（joinRoom）時に `sessionStorage.setItem('bio_edu_just_joined', 'true');` をセットする。
   - 初回入室直後のみクラウドからの初期ワークスペース展開（ダウンロード）を許可し、展開後にフラグを破棄する。以降のアプリ間遷移では上記の Anti-Rollback Guard が作動し、生徒・教員を問わずローカルの最新作業状態が最優先で保護される。
...
6. **マルチデータセットのサンプルIDベース内部結合（Inner Join）規格**:
   - 異なる解析フェーズ（Phase 2/3等）から手動バケツリレーで持ち込まれた複数のデータセットを統合するアプリでは、各行の先頭に含まれるサンプルID（`id`）をキーとして厳密な突合（マッチング）を行うこと。
   - 両方のデータセットに共通して存在するサンプルのみを抽出し、順序を揃えた新しいデータ配列を生成して統計エンジンへ渡すこと。片方にしか存在しないサンプルが検出された場合は、処理を中断せず自動除外した上で、ユーザーに警告メッセージを視覚的に通知すること。
...
**⑤ テーブル行アクティブ・ハイライト規格（Table Active Row Standard）**
テーブル内の選択状態（アクティブ行）を示す際、セルごとに `border-left` や上下枠線を付与すると、列と列の境目に余分な縦線が走り、表の罫線幅と干渉して著しい視覚的ノイズとなる。
選択行（`.active-log` 等）は、上下左右のボーダーを一切付与せず、フラットな背景色（`background-color: #d1f2eb !important;`）のみで塗りつぶすこと。ホバー時は `#f2f4f4` 等の極めて淡いグレーとし、テーブル本来のグリッド構造を美しく維持すること。
...
### 15.5 PWA Service Worker自律更新検知＆非侵入型アップデート通知UI規格

#### ① 目的と設計思想
コードデプロイおよびバージョン更新時、生徒・教員のブラウザキャッシュによる新旧混在を防ぎ、最新の生物学計算ロジックやUI改善を即座に届けるための自律更新アーキテクチャ。授業中の実験操作や考察を強制遮断しない「非侵入型（Non-blocking）UX」と、第7.4項「作業状態の完全復元」との完全連携を必須とする。

#### ② Service Worker ライフサイクル管理規格
- **更新検知トリガー**: ページロード時および定期インターバル（またはフォアグラウンド復旧時）に Service Worker の更新チェック（`registration.update()`）を実行する。
- **待機状態（waiting）検知**: バックグラウンドで新規 Service Worker がインストール完了（`state === 'installed'` かつ `navigator.serviceWorker.controller` が存在）したことを検知した時点で、更新準備完了とみなす。
- **即時アクティブ化（skipWaiting）**: ユーザーが更新アクションを実行した時のみ、Service Worker に対し `{ action: 'skipWaiting' }` を `postMessage` する（勝手な自動アクティブ化による実行中スクリプトの破断を防止）。
- **シームレスリロード**: `navigator.serviceWorker.oncontrollerchange` を監視し、新ワーカーがコントローラーとなった瞬間に `window.location.reload()` を実行する。

#### ③ 非侵入型フローティングトーストUI規格
- **ブロッキングUIの禁止**: 全画面を覆うモーダルやアラート等の強制操作ダイアログは、生徒の入力やグラフ読み取りを遮断するため固く禁ずる。
- **配置とレイアウト**: 画面下部（トースト領域、`z-index: 9999`）に浮遊するフローティングバナーとして配置する。100dvhレイアウトや下部操作ボタンと被らないよう安全マージン（`bottom: 1rem` 等）を確保する。
- **構成要素**:
  1. アイコンおよびメッセージ（例：「新しいバージョンが利用可能です」）
  2. [今すぐ更新] ボタン（視認性の高いプライマリアクセン、押下で更新実行）
  3. [後で / 閉じる] ボタン（セッション中は非表示化し、授業進行を優先可能にする）
- **デザインシステム準拠**: 既存のテーマ変数（CSS Variables: `--surface`, `--text-primary`, `--primary-accent`, `--border` 等）を使用し、ダーク/ライトテーマに完全追従させる。

#### ④ セッション永続化連携の絶対担保（Zero Data Loss）
- 第7.4項（Session-Persistent Workspace）に完全準拠すること。
- 更新リロードが実行されても、入力枠内の塩基配列・アミノ酸配列、各種パラメータ（スライダー値、選択トグル等）、解析モードが `sessionStorage` から確実に再展開され、リロード前の作業状態へ100%復元されなければならない。

#### ⑤ 実装アーキテクチャの統一
- 全アプリで個別にロジックを書かず、共通基盤モジュール `js/pwa_updater.js` およびルート `sw.js` を用いて一元管理すること。



## **大規模一括改修における開発体制とPythonスクリプト駆動SOP（トークン枯渇防止プロトコル）**

* **背景:** 全14アプリ等の複数アプリ・ファイルに対するAIの直接多重編集は、トークン上限の枯渇およびコンテキスト破綻・コード欠損を招く深刻なボトルネックとなる。
* **基本方針（Cline完全一本化）:** 開発・運用体制は「VS Code上の Cline（Actモード） ＋ Pythonスクリプト駆動型一括更新（冪等性対応）」に完全一本化し、外部AIコーディングツールの併用は行わない。
* **役割分担:**
  1. **司令塔（統括アーキテクト / Gemini）:**
     * 要望の分析とアーキテクチャ設計。
     * 日本学術会議『高等学校の生物教育における重要用語の選定について（2025年版）』への厳格な準拠チェック。
     * Pythonスクリプトの設計・仕様策定と、Cline向け「単一実行指示書」の生成。
  2. **Cline（VS Code拡張機能）の実行責務:**
...
## 第10項 ネイティブアプリライクUI/UX・端末制御規格（Native-like Mobile UX Standards）

Bio-Edu Suiteの全シミュレーション・教材Webアプリケーションにおいて、ブラウザ特有のノイズを徹底排除し、ネイティブアプリ同等の操作感と没入感を提供するため、以下の7大制御規格を標準実装とする。

### 10.1 長押し・右クリックメニューの完全封印（Context Menu制御）
* **目的:** テキスト長押しや長押しタップによるブラウザ標準のコンテキストメニュー・虫眼鏡・コピーポップアップの誤発火を防止する。
* **CSS制御:** ドキュメント全体に `-webkit-touch-callout: none;` および `user-select: none;`（`-webkit-user-select: none;`）を適用する。
* **JS制御:** ドキュメントルートで `contextmenu` イベントを監視し `preventDefault()` を実行する。ただし、ユーザーが文字・数値を編集する `<input>`、`<textarea>`、`[contenteditable="true"]` 要素内のみ例外としてメニュー表示および選択を許可する。

### 10.2 OSネイティブキーボードの最適化（Input Mode & EnterKeyHint）
* **目的:** モバイル端末での入力操作において、内容に応じた最適なソフトウェアキーボードおよびアクションボタンを即座に提供する。
* **実装要件:**
  * 数値入力欄（例: PCRの温度設定、計算値入力）には `inputmode="numeric"` または `inputmode="decimal"` を付与し、スマホの「数字専用キーパッド」を直接呼び出す。
  * 送信や検索を行う入力窓には `enterkeyhint="search"` や `enterkeyhint="go"` を指定し、キーボード右下の改行ボタンの表記をネイティブアプリ仕様に変更する。

### 10.3 スワイプバック（エッジスワイプ）の無効化
* **目的:** iOS Safari等の画面左端からのスワイプによる「前のページに戻る」ブラウザ挙動が、アプリ内の波形データ操作やドロワー開閉ジェスチャーと衝突するのを防ぐ。
* **CSS/JS制御:** 対象の操作領域や全画面に対して `overscroll-behavior-x: none;` を適用するか、必要なタッチイベント領域で `preventDefault()` を行い、ブラウザの履歴バックジェスチャーを完全に封印する。

### 10.4 スクロールバーのネイティブ化（または完全非表示）
* **目的:** WindowsやAndroidの太いブラウザ標準スクロールバーによるWeb特有の見た目を排除し、洗練された外観を保つ。
* **CSS制御:** メインコンテンツ領域のスクロールバーは `::-webkit-scrollbar` を用いて「iOS風の極細の半透明バー」にカスタマイズするか、必要に応じて完全に非表示化する。

### 10.5 ネイティブの共有機能（Web Share API）の活用
* **目的:** データの書き出しや結果の共有時に、Web特有の不自然なモーダルではなく、OS標準の共有シートを直接呼び出す。
* **JS制御:** 共有ボタンのクリック時に `navigator.share({ title, text, url })` を実行し、AirDropや各種SNS・メールアプリへのシームレスな共有を実現する。

### 10.6 画面スリープの防止（Screen Wake Lock API）
* **目的:** 「系統樹の構築」や「DNAアライメント」など、時間のかかる長時間の処理中にスマホの画面が自動暗転・ロックされるのを防ぐ。
* **JS制御:** 処理実行中は `navigator.wakeLock.request('screen')` を呼び出し、アプリ使用中の画面常時オン状態を維持する。

### 10.7 ホバー（:hover）のモバイルでの無効化（Sticky Hover対策）
* **目的:** タッチデバイスでボタンをタップした際、指を離しても `:hover` スタイル（濃色化など）が残り続ける「Sticky Hover」現象を解消する。
* **CSS制御:** `@media (hover: hover)` クエリを用いて、マウスが使用可能な環境でのみホバーエフェクトを有効化し、タッチデバイスでは沈み込み（`:active`）やハプティクス（振動）のみを反応させる。




## 第11項 統合検証ルーム・手動バケツリレーおよびアプリ連番体系マスター規格

### 11.1 正式アプリ連番体系の法制化
Bio-Edu SuiteにおけるPhase 4〜Phase 6の正式なアプリ連番体系、名称、およびテーマカラーは以下の通りに法制化する。
- ⑩ Integrative Taxonomy Studio (Phase 4)
- ⑪ Comparative Variant Analyzer (Phase 4 / テーマカラー: `#8e44ad`)
- ⑫ Statistical Genetics Lab (Phase 5 / テーマカラー: `#e74c3c`)
- ⑬ Central Dogma Simulator (Phase 6 / テーマカラー: `#e67e22`)
- ⑭ Protein Structure Explorer (Phase 6 / テーマカラー: `#e67e22`)

### 11.2 手動バケツリレー設計思想の再定義とディスパッチャー方針の是正
以前検討された「ダッシュボードへの直接的な最終データ生成ディスパッチャーの埋め込み」方針は正式に撤回・是正し、以下の教育的設計思想を厳格に遵守する。
1. **ダッシュボード（アプリ①）の役割限定**: ダッシュボード（アプリ①）は純粋なデジタル野帳（LIMS）として、生データ（未処理の塩基配列・画像等）の保管に徹する。
2. **手動バケツリレーの絶対動線**: 生徒が自らアプリ⑤・⑧（突然変異データ導出）およびアプリ⑨（形態変異データ導出）を操作して解析結果を得て、それをアプリ⑪等へ持ち寄る「手動バケツリレー」を絶対の教育的動線とする。自動的なバックグラウンド同期やブラックボックスなデータ受け渡しは一切行わない。

### 11.3 バケツリレー用ボタングループ規格と解析ステータス連動
1. **【バケツリレー用ボタングループ規格】**: 他アプリへ結果を持ち出すためのボタン群（例: アプリ⑨の「JSONコピー」「PCAコピー」「TSVコピー」、アプリ⑧の「Newickコピー」「突然変異コピー」等）は、横並びのボタングループ（`.export-btn-group`）として配置し、すべて `.btn-secondary` ⬜ を適用する。
2. **【解析ステータス連動】**: 初期状態はすべて `disabled`（非活性）とし、解析計算・描画が完了したタイミングでのみ一括して活性化させること。入力変更や再解析開始時には直ちに `disabled` へ戻すガードレールを義務付ける。

## 第16項 教育的シミュレーション・UX規格

Bio-Edu Suiteの全解析・シミュレーションアプリにおいて、単なる自動計算ツールではなく「学習者の深い思考と探究」を促すため、以下の5大UX規格を標準実装とする。

- **オンデマンドな詳細解説とソフトな足場掛け（Soft Scaffolding）**: 本格的な解析作業の円滑なフローを阻害する「解答必須のクイズ機能（ハードブロック）」は原則採用しない。代わりに、要所要所に「？」ボタン（Rationaleポップオーバー）を的確に配置して機能的・生物学的な意味を補足するほか、解析結果の傍らに「詳しい解説パネル」をオンデマンドで展開できるようにし、学生の作業を妨げずに思考をアシストする設計とすること。
- **動的解釈パネル（ダイナミック・フィードバック）**: 固定のテキストではなく、計算された統計値（$p$値やMantel $r$値等）やグラフの形状に応じた、生物学的な推測シナリオを動的に生成し提示すること。
- **ブラッシング＆リンキングによる直感連携**: 多変量データを扱うアプリでは、一方のプロット（点や枝）へのホバー操作を、関連する別のプロットへリアルタイムに連動（ハイライト）させること。
- **What-If 仮説検証と手動シミュレーション**: D3.jsを活用し、データポイントのドラッグやシークバー操作によるリアルタイム再計算・モーフィングを実装し、現象の変化を体感的に理解させること。
- **レポート作成支援**: 過保護なテキスト自動生成は行わず、背景透過・高解像度のPNG/SVG画像をワンクリックで出力できる機能を提供し、学習者自身の言葉による考察を促すこと。
- **UIの進化による学習段階の引き上げ（Progressive UI/UX）**: 複雑な機器シミュレーションにおいては、学習者の認知段階に合わせて以下の3つのUXパラダイムを切り替えられるカセット方式の導入を推奨する。
  1. **レガシー・ハードウェア型（基礎定着）**: あえて不便なテンキーやボタン操作で数値を一つ一つ手打ちさせることで、論理的なプロセスと基礎パラメーターの意味を妥協なく体に覚え込ませる。
  2. **直感・タッチスクリーン型（視覚的理解）**: グラフ上のノードを直接ドラッグ操作するなどのインタラクションを用い、テキストでは捉えにくい時間や量などの相関を視覚的に理解させ、直感的なトライ＆エラーを促す。
  3. **PCソフトウェア・高機能型（実践・俯瞰）**: 情報密度を跳ね上げ、複数ターゲットの一括設定やパラメータの俯瞰比較など、研究現場のプロフェッショナルなデータ管理と実学的な論理構築を疑似体験させる。


## 第17項 トークン極限圧縮＆ハルシネーション物理防止アーキテクチャ規格 (v35.6 制定)

AI（Gemini / Cline）を活用した持続可能な開発・運用の実現、APIコストの劇的削減、およびコンテキスト破綻・ハルシネーションを物理的に排除するため、以下の11大アーキテクチャ規格をプロジェクト全体の「絶対法」として制定する。

### 17.1 キャッシュ＆ルーティング最適化
1. **コンテキストキャッシュ規格（Context Caching）**:
   - 不変のシステム指示書、本ガイドライン、および用語規格ファイルは、サーバーサイドキャッシュを前提として設計し、リクエストごとの重複読み込みによる入力トークン消費を最大90%遮断する。
2. **モデル・ルーティング最適化規格**:
   - ロジック推論（D3.js、計算エンジン）は Pro モデル、局所パッチ・UI微調整は Flash モデル、単純テキスト修正・ガイドライン追記・ログ解析は Flash-Lite モデルへ厳格に振り分け、過剰推論によるコスト爆発を防止する。
   - [Cline側Proモデルの完全追放と手足化規格]: 高度な論理推論・生物学的整合性判定・Pythonスクリプト設計はすべて定額・無料枠のWeb側頭脳（Google AI Studio / カスタムGemのProモデル）で完結させるものとし、VS Code上のClineにはProモデル（gemini-2.5-pro等）を一切割り当てない。Clineのモデル設定は「gemini-3.5-flash-lite」または「gemini-3.7-flash」に完全固定し、指示書に記載されたスクリプト作成とコマンド実行のみを担う「手足」として極小コスト運用を徹底すること。
3. **非同期バッチ処理規格（Batch API）**:
   - 全14アプリの一括ドキュメント生成、コメント追加、オフラインテストコード一括生成など、即時性を要しない大規模処理はバッチAPI（JSONL形式）を適用し、APIコストを半額（50%OFF）に圧縮する。
4. **ハイブリッド・ルーティング規格（Local / Cloud 連携）**:
   - 構文Lintチェック、単純なCSS整形、Markdownフォーマット検証等はローカル環境（軽量ローカルLLMやスクリプト）で処理し、高度な生物学的推論や設計判断のみをクラウドAPIへ委譲して通信頻度を最小化する。

### 17.2 入出力コンテキスト極小化・保護
5. **ポインタ参照・差分指定規格（Pointer & Diff Only）**:
   - ファイル全体のコンテキスト読み込みを禁止し、行番号指定または特定ブロックの `git diff` 差分のみを入力として受け渡し、Zero-Modification（破壊なき進化）を数学的に担保する。
   - 【Web版Gemini初期共有時における特例】司令塔（Web版Gemini）の広大なコンテキストウィンドウを活かし、事前のコード共有時における全文入力は特例として許容する。ユーザーへの手作業の強要（手動トリミング等）を禁じ、AI側が自律的に骨格化・構造把握を行うこと。
6. **コード骨格化規格（Code Skeletonization）**:
   - 全体設計や依存関係の把握フェーズでは、実装ブロックを `...` に置換した「クラス・関数宣言・引数・docstring」のみの骨格テキストを生成して入力とし、数万トークンを数百トークンに圧縮する。
   - 【Web版Gemini初期共有時における特例】司令塔（Web版Gemini）の広大なコンテキストウィンドウを活かし、事前のコード共有時における全文入力は特例として許容する。ユーザーへの手作業の強要（手動トリミング等）を禁じ、AI側が自律的に骨格化・構造把握を行うこと。
7. **AST関数スコープ限定抽出規格（AST-based Function Scope Extraction）**:
   - 複雑なロジック改修時は、抽象構文木（AST）を用いて修正対象の関数スコープのみを構造的に抽出し、修正後に安全にマージする。周囲の既存コードの巻き添え破壊を物理遮断する。
8. **プロンプトXML・記号化規格（High-Density XML Protocol）**:
   - AIへの指示は冗長な自然言語を排し、`<rule>`, `<input>`, `<output>` 等のXMLタグおよび記号構造を採用してトークン密度を高め、構文解析精度を向上させる。

### 17.3 データ・ログ前処理と運用防壁
9. **前処理・ログ要約規格（Pre-processed Log Extraction）**:
   - Clineやブラウザの長大なエラーログは直接入力せず、スタックトレースの核心箇所および変更差分のみを前処理抽出して入力する。
       - 【Web版Geminiログ共有時における特例】Web版Gemini（司令塔）への不具合フィードバック時は、ユーザーに手動でのスタックトレース切り出しやエラー探索の負担を強いることを禁じ、ターミナルの生ログやCline報告書全文の直接貼付・テキスト添付を許可する。受け取った司令塔側が自律的に根本原因を特定し、Cline用には極小トークン指示書として蒸留・出力すること。

10. **巨大データ強制トランケーション規格（Data Truncation & Hash）**:
    - 長大なFASTA塩基配列・アミノ酸配列データ、巨大JSON、Base64データは、生データのままAIに入力することを厳禁とする。先頭5行・末尾5行を残し、中間を省略記号（`... [中略: 配列長N] ...`）に置換して入力する。
11. **ワンショット・コンテキストリセット規格（One-Shot Reset Protocol）**:
    - 1タスク（機能追加、パッチ適用、検証）完了ごとにチャット履歴を完全に初期化（Clear）し、過去の累積トークンによる雪だるま式課金と幻覚の継承を物理遮断する。
12. **ワークスペース自律クリーンアップ規格（Workspace Self-Cleaning Standard）**:
    - プロジェクト内の肥大化およびAIコンテキストへのゴミファイル混入を防ぐため、OS一時ファイル（.DS_Store, Thumbs.db等）の発生や、一括改修用ワンショットスクリプトの作業完了後は、`scripts/cleanup_audit.py --apply` を直ちに実行してクリーンアップおよび `scripts/archive/` への自動退避を行うこと。
...
Phase 5: Statistical Genetics Lab（アプリ⑫）において、ゲノム育種・量的遺伝学の重要概念である「組合せ能力」および「相加的・非相加的遺伝変異」を安全かつ直感的にシミュレーションするため、以下の規格を制定する。

### 18.1 ダイアレル分析の数理モデル規格
       - 【リカバリー時のチャット継続とモデル固定特例】修正・リカバリーを継続する間（Case B）は、設計方針やファイル状態を保持するためWeb側（Gemini）・Cline側ともに現在のチャット／タスクを「継続」する。モデル設定（Web側: Pro、Cline側: Flash-Lite）は一切変更せず維持すること。タスクが正常完了（Case A）した際、または致命的崩壊により白紙撤回（Case C）した際のみ、チャット履歴を完全に初期化（Clear）する。

1. **標準採用モデル（Griffing Method 2 / Model I）**:
   - 親系統 $p$ 系統およびそのF1後代（正逆交配を含まない片側交配：合計 $p(p+1)/2$ 組合せ）を対象とする「Griffingの第2法・固定効果モデル」を標準計算エンジンとする。
2. **出力指標と数理要件**:
   - **一般組合せ能力（GCA: General Combining Ability）**:
    13. **検証後ネクストアクション3分岐規格（Post-Verification 3-Way Action Protocol）**:
        - 司令塔が出力するすべての指示書末尾には、動作確認後の明確な行動指針（Case A: Git Commit & Push / Case B: ログ丸ごとリカバリー / Case C: git restore による即時白紙撤回）と、それぞれの状況における「チャットの継続／Clear」「モデル維持（Pro/Flash-Lite固定）」を必ず明記し、破壊の連鎖や作業の迷いを物理遮断すること。

     各親系統の相加的遺伝効果（相加的変異）を反映したGCA効果値 $\hat{g}_i$ を算出する。
   - **特定組合せ能力（SCA: Specific Combining Ability）**:
     特定ペア間における顕性効果（dominance effect）および非相加的相互作用を反映したSCA効果値 $\hat{s}_{ij}$ を算出する。
   - **高親ヘテロシス（High-Parent Heterosis）**:
     両親のうち高い方の親に対するF1の優越率（雑種強勢度合い）を百分率（%）で算出する。
   - **ダイアレル分散分析表（Diallel ANOVA）**:
     GCA要因、SCA要因、および誤差分散（または反復間誤差）に対する平方和（SS）、自由度（df）、平均平方（MS）、F値を算出して表形式で提示する。

### 18.2 用語規格の厳格死守（日本学術会議2025年版準拠）
- 「優性」「優性効果」「優性分散」の表記を完全禁止とする。
- ヘテロ接合体における非相加的効果およびSCA効果の解説においては、必ず「**顕性効果（dominance effect）**」「**顕性分散**」「**相加的効果（相加的変異）**」の用語を使用すること。

### 18.3 UI設計およびコンテキスト分離規格
1. **解析モード切替（Tabs/Radio）**:
   - アプリ⑫の上部に「G×E 相互作用解析（AMMI）」と「ダイアレル交配分析（Diallel）」の切り替えUIを配置し、作業コンテキストを完全に分離する。
2. **ダイアレル入力インターフェース**:
   - 親系統×親系統のハーフマトリクスグリッド入力、またはTSV貼り付け枠（`Parent_1`, `Parent_2`, `Trait_Value`）を採用し、データフォーマット不一致時のアシストパネルを配備する。
3. **可視化モジュール**:
   - 親系統ごとのGCA効果棒グラフ、組合せごとのSCA効果ヒートマップ／散布図、およびダイアレルANOVA表を統合提示する。
