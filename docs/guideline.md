# **🎨 Bio-Edu Suite 統合UI/UX・開発ガイドライン (v37.1_2026-09-27:最新版)**
...
## **【改訂・サルベージ履歴】**
* **v37.1 (2026-09-27):** スマート初期化・ルーム解散ハイブリッド規格、およびクラウド同期・15万文字画像許容枠規格の法制化。
  - [スマート初期化・ルーム解散ハイブリッド規格]: ドロワーから「データを初期化」を実行した際、教員ロール（TEACHER-*）の場合はFirestore上のルーム全体（生徒データ含む）を一括消去して解散し、生徒ロールの場合は自身の作業データスナップショットのみを消去して退出する分離ロジックを義務化。再入室時のゾンビ復活を物理遮断する。
  - [クラウド同期・15万文字画像許容枠規格]: `auth_sync.js`のワークスペース自動同期およびマスター課題登録において、150,000文字（約110KB）以下の軽量な画像データは同期対象として保持し、これを超える超巨大画像のみをパージしてローカルマージエンジンへ委譲する安全閾値を明文化。
* **v37.0 (2026-09-23):** 階層型サンプルハブ規格（Multi-file Sample Hub Standard）およびスマート・ディスパッチ規格の法制化。
  - [階層型サンプルデータスキーマ規格]: 従来の1行＝1配列のフラット構造を排し、1サンプル（エンティティ）に対し複数のサンガー波形（Forward/Reverseの.ab1）、複数の標的遺伝子配列（CO1, Cytb等のFASTA）、形態画像、および環境メタデータを一元集約する階層型（Nested）データ構造を制定。
  - [スマート・ディスパッチ（マーカー選択送信）規格]: ダッシュボードから解析アプリ（④ Sanger Trace Editor、⑤ DNA Alignment Studio等）へデータを送信する際、複数遺伝子が混在するマルチFASTAの誤送信を防止するため、送信対象マーカーを選択・抽出して安全にバケツリレーを行う調停パイプラインを義務化。
* **v36.9 (2026-09-23):** 入室中オンデマンド課題読込パネル規格（On-Demand Task Import Standard）の法制化。
  - [入室中読込窓の常設]: ルーム入室中（#modalLoggedInView）においても、生徒および教員が授業進行に合わせて任意のタイミングで課題コードを入力・即時展開できるよう、全アプリ共通の #accountModal 内に課題読込パネルを常設することを義務化。
  - [Firestoreキーサニタイズ完全自動化]: セッションデータ保存時にキー名に含まれる '.' を '__dot__' へ自動変換し、受信時に復元することで、ネストフィールド誤認エラー（invalid nested entity）を物理遮断するパイプラインを明文化。
* **v36.8 (2026-09-23):** LWW（最終更新時刻優先）調停エンジン規格、教員クロスデバイス作業継続規格、および非破壊画像保護・ローカルマージ規格の法制化。
  - [LWW調停エンジン規格]: ローカルにデータが存在するか否かの曖昧な判定を全廃し、ユーザーの手動編集時刻（`bio_edu_last_user_edit`）とクラウド更新時刻（`lastUpdated` / `teacherUpdatedAt`）のタイムスタンプ比較に基づく自律調停を義務化。クラウドが新しい場合は教員・生徒を問わず完全ハイドレーションを実行する。
  - [教員クロスデバイス作業継続規格]: 教員IDであっても「配信専用親機」として受信を拒否することを禁止し、別端末・別ウィンドウからの初回入室時や他端末での更新検知時にはクラウドから最新の作業空間（ダッシュボード含む全14アプリ）を完全復元することを保証する。
  - [非破壊画像保護＆ローカルマージ規格]: 通信容量制約への対応において、ローカルストレージ（`localStorage` / `sessionStorage`）の実データを破壊・空文字化することを厳禁とし、クラウド側画像が軽量化されている場合でもローカルの元画像を自動マージして復元するフェイルセーフを義務化。
* **v36.7 (2026-09-23):** Firestoreペイロードのドットパス制約回避・キーサニタイズ義務の法制化。
  - [キーサニタイズ規格]: `sessionStorage` のキー名に含まれる `.`（ドット、例: `_index.html`）は Firestore の `setDoc({ merge: true })` において「ネストされたフィールドパス」と誤認され `invalid nested entity` エラーを引き起こすため、送信時は `.replace(/\.html$/, '_html')` でサニタイズし、受信時は `restoreKeyName(safeKey)` で元の `.html` キー名へ復元するサイクルを厳守。
* **v36.6 (2026-09-23):** 環境メタデータ動的カラーマッピング規格、絵文字フリー学術凡例（Legend）規格、およびPhase 1→Phase 4/5共通TSV連携規格の法制化。
  - [環境メタデータ動的カラーマッピング規格]: アプリ⑩（Integrative Taxonomy Studio）において、外部メタデータ（環境TSV）を受け入れ、タングルグラムのリーフノード（円・文字）およびPCA散布図のプロット点を環境要因ごとに動的着色する機能を標準化。
  - [絵文字フリー学術凡例規格]: 幼稚な装飾絵文字を全廃し、SVG上部に「半径5pxの塗りつぶし円 ＋ 11px太字テキスト」を横並びで整然と描画する学術凡例（.env-legend）のデザインシステムを制定。
  - [Phase 1→Phase 4/5共通TSV連携規格]: ダッシュボード（アプリ①）の「⑩・⑫ 環境TSVをコピー」から出力される3列TSV（Sample_ID \t 系統名 \t 環境カテゴリ）を、アプリ⑩（系統樹色分け）およびアプリ⑫（統計遺伝学GxE解析）の双方が直接受け入れ可能な共通データパイプラインとして法制化。
  - [ハイブリッド入力調停エンジン規格]: アプリ⑩の第4入力枠において、2列目の値が形態系統樹のリーフ名と一致する場合は「宿主-寄生虫対応表」、一致しない場合は「環境メタデータ」として自律判別・処理する共存ロジックを義務化。
...
* **Phase 1 → Phase 4/5 共通環境TSV連携パイプライン**:
  ダッシュボード（アプリ①）に登録された「環境データ」は、アプリ⑫（Statistical Genetics Lab）の環境(E)要因としてのみならず、アプリ⑩（Integrative Taxonomy Studio）のタングルグラムおよびPCA空間における環境カラーマッピングのメタデータとしても機能する。データ出力は `# Source` コメント行付きの3列TSV（`Sample_ID \t 系統・個体群 \t 環境カテゴリ`）に標準化し、受入先アプリでヘッダーの自動スキップおよび柔軟なID/名称突合を行うこと。
...
4. **初期化処理のハンドリング**:
   ...
   - また、リロード時のオートセーブ逆流（レースコンディション）を防ぐため、`clear()` 実行直前に `window.isResetting = true;` を宣言し、さらに `Storage.prototype.setItem = function() {};` を記述してブラウザの保存機能を一時的に無力化（物理遮断）することを義務付ける。
    ...
    4. **教員・生徒ハイブリッド巻き戻し防止機構（Anti-Rollback Guard）およびLWW調停エンジン規格**:
       - ローカルにデータが存在するか否かの曖昧な判定を全廃し、ユーザーの手動編集時刻（`bio_edu_last_user_edit`）とクラウド更新時刻（`lastUpdated` / `teacherUpdatedAt`）のタイムスタンプ比較に基づく自律調停を義務化。クラウド側のタイムスタンプがローカルよりも新しい場合は、教員・生徒を問わず完全ハイドレーションを実行する。
       - 同一セッション内に有効な作業データ（`bio_edu_workspace_*`, `bio_edu_ws_*`, `bio_edu_draft_*`）が存在し、かつローカル編集時刻がクラウドより新しい場合、アプリ間遷移時のクラウド同期（ダウンロード）による過去スナップショットでの巻き戻し・上書き・ゾンビ復活を物理遮断し、常にローカル最新をクラウドへ同期保存して早期リターンする。
       - **教員クロスデバイス作業継続規格**: 教員ID（`TEACHER-*`）であっても「配信専用親機」として受信を拒否することを禁止し、別端末・別ウィンドウからの初回入室時や他端末での更新検知時にはクラウドから最新の作業空間（ダッシュボード含む全14アプリ）を完全復元することを保証する。
       - **非破壊画像保護＆ローカルマージ規格**: 通信容量制約への対応において、ローカルストレージ（`localStorage` / `sessionStorage`）の実データを破壊・空文字化することを厳禁とし、クラウド側画像が軽量化されている場合でもローカルの元画像を自動マージして復元するフェイルセーフを義務化。この際、クラウド同期の対象となる画像データの安全閾値は 150,000文字（約110KB）とし、これを下回る軽量な画像は同期対象として保持し、超える巨大画像のみを間引くこと。
...
4. スマート初期化・ルーム解散ハイブリッド規格（v37.1 制定）:
   - 従来の「ローカルのみ削除（クラウド保護）」の仕様をアップデートし、「不慮の事故による離脱（自動復旧）」と「ユーザーの意図的な初期化（データ削除）」を完全に区別する。
   - 【生徒・通常ユーザー（個人のクラウドデータ削除）】: ドロワー等から「データを初期化（`confirmDataReset`）」を実行した際、ローカルストレージのクリアに加えて、Firestore上にある「自分（該当参加者ID）」の作業データ・スナップショットのみを削除（空箱化）するリクエストを送信してルームを退出する。これにより再入室時のゾンビ復活を防止する。他者のデータや教員マスターデータには影響を与えない。
   - 【教員・TEACHER-*（ルーム完全解散）】: 教員ロールで「ルーム解散して初期化」を実行した際、ローカルのクリアに加えて、Firestore上の該当ルーム（`rooms/{roomCode}`）ドキュメント全体に対して削除リクエストを送信し、配下にある全生徒のデータや教員ライブステートをすべて一括消去（パージ）してルームを閉じる。これにより授業後の情報セキュリティ的クリーンアップを実現する。
...
2. **教員端末におけるクロスデバイス受信用リスナー有効化および最新ステート復元義務**:
   - 教員端末であっても「配信専用親機」としてリモートからの同期受信を拒否することを厳禁とし、自宅PC・学校PC等のクロスデバイス環境や別ウィンドウでの入室時、および他端末での更新検知時には、クラウドから最新の作業空間（ダッシュボード含む全14アプリ）を完全復元（ハイドレーション）するリスナーを常時有効化することを義務付ける。
...
### 7.7 階層型サンプルデータスキーマおよびマルチファイル管理規格（Multi-file Sample Hub Standard）
- **目的**: 単一遺伝子の解析にとどまらず、マルチジーン解析（CO1＋Cytb等）やサンガー波形コンセンサス決定（Forward/Reverseリード）を円滑に行うため、ダッシュボードを本格的なLIMS（実験室情報管理システム）へと進化させる。
- **階層型データ構造（Nested Entity）**:
  ダッシュボードで管理する各サンプルは、以下の階層構造を持つJSONオブジェクトとして定義する。
  1. `id`: 一意のサンプルID（例: `Sample-01`）
  2. `name` / `species`: 生物名または系統名（日本学術会議2025年版に準拠）
  3. `metadata`: 採取地、環境要因、生息場所（ハビタット）などのデジタル野帳情報
  4. `traces`: サンガーシーケンス波形データ群（.ab1。リード方向 `direction: 'forward' | 'reverse'`、遺伝子マーカー名を含む）
  5. `sequences`: 確定した塩基配列・アミノ酸配列群（FASTA。マーカー遺伝子タグ `marker: 'CO1' | 'Cytb' | ...` を含む）
  6. `images`: 形態測定・同定用画像データ群（Base64またはObject URL）
- **非破壊ストレージ原則の堅持**:
  複数ファイルのアタッチや削除操作時においても、端末内の `localStorage` / `sessionStorage` 上の実データを破壊・誤消去せず、常に最新のツリー構造として保全すること。

### 7.8 スマート・ディスパッチ（マーカー選択送信）規格
- **目的**: 1サンプル内に複数マーカー（CO1, Cytb等）が共存する場合に、すべての塩基配列を盲目的に結合してアライメントツールへ送信することによる生物学的過誤（異種相同遺伝子の混同）を未然に防止する。
- **送信調停UI（モーダル）の義務化**:
  ダッシュボード下部の一括送信ボタン（例: [⑤ FASTAを送る]）を押下した際、対象サンプル内に複数種類のマーカー遺伝子が存在する場合は、直ちに送信せず「送信対象マーカーの選択モーダル（例: ☑ CO1 のみ送信）」を表示すること。
- **安全なマルチFASTA生成**:
  選択されたマーカーの塩基配列のみを抽出し、適切なFASTAヘッダー（`>[Sample_ID]_[Marker] [Species]`）を付与した上で `sessionStorage` に格納し、受入先アプリへ遷移させること。
...
- **タングルグラム学術凡例（Legend）および動的カラーマッピング規格**:
  タングルグラムにおいて外部メタデータ（環境要因・地理的分布等）を可視化する際、末端ノードの円および生物種名テキストをカテゴリカラーで塗り分けること。上部には絵文字を一切使わず、SVGの `<circle r="5">` と `<text>` による洗練されたインライン凡例を横並びで自動描画し、収斂進化や適応放散の要因を視覚的に一撃で理解させる足場掛けを提供すること。
