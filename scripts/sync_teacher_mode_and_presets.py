...
def patch_auth_sync():
    file_path = 'auth_sync.js'
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Update activeTextarea selector in importMasterPreset
    content = re.sub(
        r'const activeTextarea = document\.querySelector\([^)]+\);',
        r"const activeTextarea = document.querySelector('textarea.paste-area, textarea#dnaInput, textarea#fastaInput, textarea#chain-code-input, textarea#pasteArea, input#pdbId');",
        content
    )
    
    # Add custom event dispatch for bio_edu_preset_loaded
    if "new CustomEvent('bio_edu_preset_loaded'" not in content:
        # We need to insert it right after the sessionStorage restore part.
        restore_pattern = r'(\s*sessionStorage\.setItem\(k,\s*data\.payload\.sessionData\[k\]\);\s*\}\);\s*\})'
        replacement = r"\1\n        window.dispatchEvent(new CustomEvent('bio_edu_preset_loaded', { detail: { taskCode: cleanCode, payload: data.payload } }));"
        content = re.sub(restore_pattern, replacement, content)
        
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print("Patched auth_sync.js")
...
"""
    ...
    for file in html_files:
        ...
        new_content, n = script_pattern.subn(SCRIPT_REPLACEMENT, content)
